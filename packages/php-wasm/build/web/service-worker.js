import { i as isURLScoped, t as toRelativeUrl, r as removeURLScope, s as setURLScope, g as getURLScope, a as awaitReply, b as getNextRequestId } from './urls-4b565ba2.js';
...
function initializeServiceWorker(config) {
    ...
    self.addEventListener('fetch', (event) => {
        ...
        console.debug(`[ServiceWorker] Serving request: ${toRelativeUrl(removeURLScope(url))}`);
...
async function convertFetchEventToPHPRequest(event) {
    ...
    try {
        ...
        console.debug('[ServiceWorker] Forwarding a request to the Worker Thread', { message });
        const requestId = await broadcastMessageExpectReply(message, getURLScope(url));
...
export { broadcastMessageExpectReply, cloneRequest, convertFetchEventToPHPRequest, initializeServiceWorker, seemsLikeAPHPServerPath };
