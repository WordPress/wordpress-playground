...
export const dependenciesTotalSize = 12655915;
export function init(RuntimeName, PHPLoader) {
	...
	var PHPWASM = {
		init: function () {
			...
			FS.mkdir('/experimental-sabfs');
			const S_IFCHR = 0o020000, S_IFDIR = 0o040000, S_IFREG = 0o100000, S_IFLNK = 0o120000;
			const MODE_DIR = S_IFDIR | 0o777, MODE_FILE = S_IFREG | 0o666, MODE_SYMLINK = S_IFLNK | 0o777;

			const IDX_LOCK = 0, IDX_MAGIC = 1, IDX_NEXT_INODE = 2, IDX_NEXT_DATA = 3;
			const HEADER_WORDS = 256;
			const I_MODE = 0, I_UID = 1, I_GID = 2, I_NLINK = 3, I_SIZE_LO = 4, I_SIZE_HI = 5,
				I_PARENT = 6, I_DATA_OFF = 7, I_CAPACITY = 8, I_RDEV = 9, I_ATIME_LO = 10,
				I_ATIME_HI = 11, I_MTIME_LO = 12, I_MTIME_HI = 13, I_CTIME_LO = 14, I_CTIME_HI = 15,
				I_NAME_OFF = 16;
			...
			// ───────────────── backend factory ─────────────────
			function SharedSABFS(FS, metaBuf, dataBuf, maxInodes = 4096) {
			...
			// ── locking ──
			const lock   = () => { while (Atomics.compareExchange(meta, IDX_LOCK, 0, 1) !== 0) Atomics.wait(meta, IDX_LOCK, 1); };
			const unlock = () => { Atomics.store(meta, IDX_LOCK, 0); Atomics.notify(meta, IDX_LOCK); };
			...
			function writeTimes(off, at = false, mt = false, ct = false) {
				const t = now64();
				if (at) { meta[off + I_ATIME_LO] = Number(t & 0xffffffffn); meta[off + I_ATIME_HI] = Number(t >> 32n); }
				if (mt) { meta[off + I_MTIME_LO] = Number(t & 0xffffffffn); meta[off + I_MTIME_HI] = Number(t >> 32n); }
				if (ct) { meta[off + I_CTIME_LO] = Number(t & 0xffffffffn); meta[off + I_CTIME_HI] = Number(t >> 32n); }
			}
			function allocData(bytes) {
			...
			// dir helpers
			const dirCount  = (off) => meta[off + I_SIZE_LO];
			const dirVec    = (off) => new Int32Array(metaBuf, meta[off + I_DATA_OFF], dirCount(off));
			const dirPush   = (off, id) => {
				let cnt = dirCount(off); let cap = meta[off + I_CAPACITY]; let start = meta[off + I_DATA_OFF];
				if (cnt >= cap) {                          // grow vector
				const nc = Math.max(4, cap * 2);
				const n  = allocData(nc * 4);
				if (cnt) meta8.copyWithin(n, start, start + cnt * 4);
				start = n; cap = nc; meta[off + I_DATA_OFF] = start; meta[off + I_CAPACITY] = cap;
				}
				new Int32Array(metaBuf, start, cap)[cnt] = id;
				meta[off + I_SIZE_LO] = cnt + 1;
			};
			const dirRemove = (off, id) => {
				const cnt = dirCount(off); const vec = dirVec(off);
				for (let i = 0; i < cnt; i++) if (vec[i] === id) { vec[i] = vec[cnt - 1]; meta[off + I_SIZE_LO] = cnt - 1; return; }
				throw new FS.ErrnoError(44);
			};

			// inode alloc
			function allocInode(parent, mode, name, rdev = 0) {
				lock();
				const id = Atomics.add(meta, IDX_NEXT_INODE, 1);
				if (id > maxInodes) { unlock(); throw new FS.ErrnoError(28); }
				const off = ioff(id);
				meta.fill(0, off, off + INODE_WORDS);
				meta[off + I_MODE] = mode; meta[off + I_PARENT] = parent; meta[off + I_NLINK] = 1; meta[off + I_RDEV] = rdev;
				writeSize(off, 0); writeName(off, name); writeTimes(off, true, true, true);
				if ((mode & S_IFDIR) === S_IFDIR) meta[off + I_CAPACITY] = 0; // dir vector lazily alloc
				dirPush(ioff(parent), id); writeTimes(ioff(parent), false, true, true);
				unlock();
				return id;
			}

			// raw I/O
			function readData(id, pos, len, out) {
				const off = ioff(id); const size = readSize(off); if (pos >= size) return 0;
				const to  = Math.min(len, size - pos);
				out.set(data8.subarray(meta[off + I_DATA_OFF] + pos, meta[off + I_DATA_OFF] + pos + to));
				writeTimes(off, true, false, false); return to;
			}
			function writeData(id, pos, src) {
				const off = ioff(id); let cap = meta[off + I_CAPACITY]; let start = meta[off + I_DATA_OFF];
				let size = readSize(off); const end = pos + src.length;
				lock();
				if (end > cap) {                              // reallocate
				const newCap = Math.max(end, cap ? (cap < 1 << 20 ? cap * 2 : (cap * 9) >> 3) : 256);
				const newOff = allocData(newCap);
				if (size) data8.copyWithin(newOff, start, start + size);
				start = newOff; cap = newCap; meta[off + I_DATA_OFF] = start; meta[off + I_CAPACITY] = cap;
				}
			...
			// mark deleted
			const markDel = (id) => { meta[ioff(id) + I_MODE] = 0; };

			// JS‑side node creator
			function makeNode(id, mount, parent) {
				const off = ioff(id);
				...
				node.stream_ops = FS.isChrdev(mode) ? FS.getDevice(node.rdev).stream_ops : stream_ops;
				return node;
			}
			...
			const node_ops = {
				getattr(node) {
				const o = ioff(node.sabId), mode = meta[o + I_MODE];
				const sz = (mode & S_IFDIR) ? 4096 : readSize(o);
				const ts = (i) => new Date(meta[o + i + 1] * 0x100000000 + meta[o + i]);
				return { dev: 1, ino: node.sabId, mode, nlink: meta[o + I_NLINK], uid: meta[o + I_UID], gid: meta[o + I_GID],
						rdev: meta[o + I_RDEV], size: sz, atime: ts(I_ATIME_LO), mtime: ts(I_MTIME_LO), ctime: ts(I_CTIME_LO) };
				},
				setattr(node, a) {
				const o = ioff(node.sabId); lock();
				if (a.mode !== undefined) meta[o + I_MODE] = (meta[o + I_MODE] & 0o170000) | (a.mode & 0o777);
				if (a.uid !== undefined)  meta[o + I_UID]  = a.uid;
				if (a.gid !== undefined)  meta[o + I_GID]  = a.gid;
				if (a.size !== undefined) {
					const cur = readSize(o); if (a.size < cur) writeSize(o, a.size);
					else if (a.size > cur) writeData(node.sabId, cur, new Uint8Array(a.size - cur));
				}
				writeTimes(o, false, false, true); unlock();
				},
				lookup(parent, name) {
				const vec = dirVec(ioff(parent.sabId));
				for (let i = 0; i < vec.length; i++) if (readName(ioff(vec[i])) === name) return makeNode(vec[i], parent.mount, parent);
				throw new FS.ErrnoError(44);
				},
				mknod(p, n, m, d) { return makeNode(allocInode(p.sabId, m, n, d), p.mount, p); },
				rename(old, ndir, n) {
				lock(); dirRemove(ioff(meta[ioff(old.sabId) + I_PARENT]), old.sabId);
				meta[ioff(old.sabId) + I_PARENT] = ndir.sabId; writeName(ioff(old.sabId), n);
				dirPush(ioff(ndir.sabId), old.sabId); writeTimes(ioff(old.sabId), false, false, true);
				unlock();
				},
				unlink(p, n) { const c = node_ops.lookup(p, n); lock(); dirRemove(ioff(p.sabId), c.sabId); markDel(c.sabId); unlock(); },
				rmdir(p, n)  { const c = node_ops.lookup(p, n); if (dirCount(ioff(c.sabId))) throw new FS.ErrnoError(55);
											lock(); dirRemove(ioff(p.sabId), c.sabId); markDel(c.sabId); unlock(); },
				readdir(n) { const l = [".", ".."], v = dirVec(ioff(n.sabId)); for (let i=0;i<v.length;i++) l.push(readName(ioff(v[i]))); return l; },
				symlink(p, n, tgt) { const id = allocInode(p.sabId, MODE_SYMLINK, n); writeData(id,0,TE.encode(tgt)); return makeNode(id,p.mount,p); },
				readlink(n) { if (!(n.mode & S_IFLNK)) throw new FS.ErrnoError(28);
								const o = ioff(n.sabId); return TD.decode(data8.subarray(meta[o+I_DATA_OFF], meta[o+I_DATA_OFF]+readSize(o))); },
				chmod(n, m) { node_ops.setattr(n,{mode:m}); },
				chown(n, u, g) { node_ops.setattr(n,{uid:u,gid:g}); }
			};

			// ── stream_ops ──
			const stream_ops = {
				open(s) { if ((s.flags & 0x400) && FS.isFile(s.node.mode)) node_ops.setattr(s.node,{size:0});
							s.position = (s.flags & 0x800) ? readSize(ioff(s.node.sabId)) : 0; },
				close() {},
				read(s,b,o,l,p){const pos=p??s.position,tmp=new Uint8Array(l);
				const r=readData(s.node.sabId,pos,l,tmp);b.set(tmp.subarray(0,r),o);if(p===undefined)s.position+=r;return r;},
				write(s,b,o,l,p){const pos=p??s.position;
				writeData(s.node.sabId,pos,b.subarray(o,o+l));if(p===undefined)s.position+=l;return l;},
				llseek(s,off,w){let pos=w===0?off:w===1? s.position+off: readSize(ioff(s.node.sabId))+off;
				if(pos<0)throw new FS.ErrnoError(28); s.position=pos; return pos;},
				flush(){return 0;}, fsync(){return 0;}, ioctl(){throw new FS.ErrnoError(59);},
				allocate(s,o,l){node_ops.setattr(s.node,{size:o+l});},
				mmap(s,addr,len,pos,prot,flags){const p=FS._malloc(len);
				new Uint8Array(FS.HEAPU8.buffer,p,len).set(new Uint8Array(len)); stream_ops.read(s,new Uint8Array(FS.HEAPU8.buffer,p,len),0,len,pos);
				return {ptr:p,allocated:true};},
				msync(){return 0;}
			};

			// backend object
			return { mount: (m) => makeNode(1, m, null) };
			}

			// ───────────────── convenience helper ─────────────────
			const createSharedFSBuffers = (metaBytes = 8 << 20, dataBytes = 16 << 20) =>
			({ metaBuf: new SharedArrayBuffer(metaBytes), dataBuf: new SharedArrayBuffer(dataBytes) });
			const { metaBuf, dataBuf } = createSharedFSBuffers();
			FS.mount(SharedSABFS(FS, metaBuf, dataBuf), {}, "/experimental-sabfs");

			// Example of creating a new file and reading it
			const testFilePath = "/experimental-sabfs/test.txt";
			
			console.log(FS.writeFile(testFilePath, 'hey', {encoding: 'utf8'}));
			console.log(FS.readFile(testFilePath, { encoding: 'utf8' }));
			
			// Create a new worker inline
			const workerCode = `
				self.onmessage = function(e) {
					const { metaBuf, dataBuf } = e.data;
					
					// Access the shared buffers
					const meta = new Int32Array(metaBuf);
					const data = new Uint8Array(dataBuf);
					
					// Read the test file data from the shared buffer
					// Find the inode for the test file (this is simplified)
					let fileContent = '';
					try {
						// Log the first few bytes of the data buffer to see what's stored
						const dataView = data.slice(0, 100);
						
						// Send the data back to the main thread
						self.postMessage({
							success: true,
							message: 'Worker accessed shared buffer',
							dataPreview: Array.from(dataView)
						});
					} catch (error) {
						self.postMessage({
							success: false,
			...
			worker.onmessage = function(e) {
				...
				if (e.data.dataPreview) {
					console.log('Data preview from shared buffer:', 
						new TextDecoder().decode(new Uint8Array(e.data.dataPreview.filter(byte => byte !== 0))));
				}
			};
			
			// Send the shared buffers to the worker
			worker.postMessage({
				metaBuf: metaBuf,
				dataBuf: dataBuf
			});
			
			// Clean up the URL when done (optional)
			setTimeout(() => {
