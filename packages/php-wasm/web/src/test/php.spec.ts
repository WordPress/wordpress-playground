...
import { PHP, loadPHPRuntime } from '@wordpress/php-wasm-common';
...
describe('PHP – stdio', () => {
  ...
  it('should output strings (2) ', () => {
    expect(php.run({ code: '<?php echo "Hello world!\nI am PHP";' })).toEqual({
      headers: expect.any(Object),
      httpStatusCode: 200,
      body: new TextEncoder().encode('Hello world!\nI am PHP'),
      errors: '',
...
--boundary--`,
    ...
    expect($_SERVER).toHaveProperty('HTTP_HOST', 'https://example.com:1235');
    expect($_SERVER).toHaveProperty('SERVER_NAME', 'https://example.com:1235');
