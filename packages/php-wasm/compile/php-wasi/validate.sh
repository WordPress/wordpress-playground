...
grep -Fq "import wasi:sockets/network@$WASI_WIT_VERSION;" "$resolved_wit"
grep -Fq "import wasi:sockets/instance-network@$WASI_WIT_VERSION;" "$resolved_wit"
grep -Fq "import wasi:sockets/tcp@$WASI_WIT_VERSION;" "$resolved_wit"
grep -Fq "import wasi:sockets/tcp-create-socket@$WASI_WIT_VERSION;" "$resolved_wit"
grep -Fq "import wasi:sockets/ip-name-lookup@$WASI_WIT_VERSION;" "$resolved_wit"
...
echo "Validated $(basename "$COMPONENT"): WASI $WASI_WIT_VERSION reactor, PHP handler/CLI, TCP/DNS, output, lock, and SQLite WAL SHM imports"
