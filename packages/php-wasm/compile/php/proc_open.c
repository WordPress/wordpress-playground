...
static void proc_open_rsrc_dtor_legacy(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	struct php_process_handle *proc = (struct php_process_handle*)rsrc->ptr;
	if (proc) {
		/* Capture is_persistent before we free the containing struct —
		 * pefree(proc, proc->is_persistent) would read a freed field. */
		int is_persistent = proc->is_persistent;
		if (proc->pipes) {
			pefree(proc->pipes, is_persistent);
		}
		if (proc->command) {
			pefree(proc->command, is_persistent);
		}
		if (proc->env.envarray) {
			pefree(proc->env.envarray, is_persistent);
		}
		if (proc->env.envp) {
			pefree(proc->env.envp, is_persistent);
		}
		pefree(proc, is_persistent);
	}
}
