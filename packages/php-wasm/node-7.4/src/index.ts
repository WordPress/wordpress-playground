...
export async function getIntlExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(currentDirPath, 'jspi/extensions/intl/7_4/intl.so');
	} else {
		return join(currentDirPath, 'asyncify/extensions/intl/7_4/intl.so');
	}
}

export async function getXdebugExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(currentDirPath, 'jspi/extensions/xdebug/7_4/xdebug.so');
	} else {
		return join(currentDirPath, 'asyncify/extensions/xdebug/7_4/xdebug.so');
	}
}
