...
describe('phpVar', () => {
	...
	test('escapes single quotes in strings (simple)', () => {
		...
		expect(result).toBe(`json_decode('"This is Playground\\'s test suite"')`);
	});

	test('escapes single quotes in strings (complex)', () => {
		const result = phpVar(`This is an escaped quot: \\'. This is an escaped backslash and a quot: \\\\'. This is an evil terminating backslash: \\`);
		expect(result).toBe(`json_decode('"This is an escaped quot: \\\\\\\\\\'. This is an escaped backslash and a quot: \\\\\\\\\\\\\\\\\\'. This is an evil terminating backslash: \\\\\\\\"')`);
	});
	...
	test('translates objects', () => {
		...
		expect(result).toBe(`array(json_decode('"a"') => 5, json_decode('"b"') => json_decode('"test"'))`);
	});

	test('translate nested arrays', () => {
		const result = phpVar([1, ['a', 'b'], 3]);
		expect(result).toBe(`array(1, array(json_decode('"a"'), json_decode('"b"')), 3)`);
	});

	test('translate nested objects', () => {
		const result = phpVar({ outer: { inner: 'value' } });
		expect(result).toBe(`array(json_decode('"outer"') => array(json_decode('"inner"') => json_decode('"value"')))`);
	});
