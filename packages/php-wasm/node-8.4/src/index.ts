...
export async function getIntlExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(__dirname, '../jspi/extensions/intl/8_4/intl.so');
	} else {
		return join(__dirname, '../asyncify/extensions/intl/8_4/intl.so');
	}
}

export async function getXdebugExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(__dirname, '../jspi/extensions/xdebug/8_4/xdebug.so');
	} else {
		return join(__dirname, '../asyncify/extensions/xdebug/8_4/xdebug.so');
	}
}
