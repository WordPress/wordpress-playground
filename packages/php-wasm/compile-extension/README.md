...
await loadPHPExtension(php, {
	source: {
		format: 'manifest',
		url: 'https://example.com/wp_mysql_parser/manifest.json',
	},
});
```
...
Call `loadPHPExtension()` before the first `php.run()` or request so the helper
can install the preload script before user code runs.

Blueprints use the same loader through the `loadPHPExtension` step:
...
For a raw `.so`, pass a file resource and, when the filename is not
`<extension-name>.so`, set `name`:
...
```json
{
	...
	"source": {
		"resource": "url",
		"url": "https://example.com/extensions/wp_mysql_parser.so"
	},
	"name": "wp_mysql_parser"
}
```
...
Xdebug. Use `extraFiles` for sidecar files needed by the extension:
...
```json
{
	...
	"source": {
		"resource": "url",
		"url": "https://example.com/extensions/my_extension.so"
	},
	"name": "my_extension",
	"extraFilesPath": "/internal/shared/my_extension",
	"extraFiles": {
		"resource": "literal:directory",
		"name": "my-extension-assets",
		"files": {
			"config.txt": "sidecar data"
		}
	},
	"env": {
		"MY_EXTENSION_DATA_DIR": "/internal/shared/my_extension"
	}
}
```

For binary sidecars, use a `bundled` resource from a `blueprint.zip` or a
`git:directory` resource instead of embedding the bytes in JSON.

When a manifest is loaded from a URL resource, relative artifact paths resolve
against the manifest URL. For bundled or literal manifests, set
`manifestBaseUrl` or use absolute artifact URLs.
