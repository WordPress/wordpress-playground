...
export const dependenciesTotalSize = 17917503;
...
/** not-t@type {!BigUint64Array} */ HEAPU64,
	...
	function _fd_close(fd) {
		...
		if (lock_utils.maybeLockedFds.has(fd) && pathResolutionErrno === 0) {
			...
			const nativeFilePath =
				lock_utils.get_native_path_from_vfs_path(vfsPath);
	...
	var lock_utils = {
		...
		is_nodefs_path(path) {
			...
			if (!answer && path.includes('.ht.sqlite')) {
				// TODO: Re-enable this after merge into the PR
				// js_wasm_trace(`is_nodefs_path ${path} is_nodefs_node ${answer} node ${nodeUtil.inspect(node, {
				//   depth: 2
				// })}`);
			}
			return answer;
	...
	function _js__syscall_fcntl64(fd, cmd, varargs) {
	...
	function __asyncjs__call_js__syscall_fcntl64(fd, cmd, args) {
		return Asyncify.handleAsync(async () => {
	...
	__asyncjs__call_js__syscall_fcntl64.sig = 'iiii';

	var wasmImports = {
		/** @export */ __assert_fail: ___assert_fail,
		/** @export */ __asyncjs__call_js__syscall_fcntl64,
		/** @export */ __asyncjs__js_fd_read,
