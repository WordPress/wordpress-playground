...
import { PHP, loadPHPRuntime, PHPServer } from '@php-wasm/common';
describe.each(PHP_VERSIONS)('[PHP %s] PHPServer – request', (phpVersion) => {
	let php: PHP;
	let phpServer: PHPServer;
	beforeEach(async () => {
	...
	it('should execute a PHP file', async () => {
		...
		const response = await phpServer.request({
	...
	it('should only handle a single PHP request at a time', async () => {
		...
		const response1 = phpServer.request({
			relativeUrl: '/index.php',
		});
		expect(phpServer.isRequestRunning).toBe(true);
		// No stdout should be written yet
		expect(php.fileExists('/tmp/stdout')).toBe(false);
		const response2 = phpServer.request({
			relativeUrl: '/index.php',
		});
