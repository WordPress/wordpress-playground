...
declare module 'fs-ext-extra-prebuilt' {
	/**
	...
	export function lockFileEx(
		fd: number,
		flags: number,
		// TODO: Consider converting to bigint and using one field each for offset and length.
		offsetLow: number,
		offsetHigh: number,
		lengthLow: number,
		lengthHigh: number,
	...
	export namespace constants {
		...
		/**
		 * Windows LockFileEx flag for requesting an exclusive lock
		 */
		const LOCKFILE_EXCLUSIVE_LOCK: number;
		/**
		 * Windows LockFileEx flag for failing immediately if the lock cannot be acquired
		 */
