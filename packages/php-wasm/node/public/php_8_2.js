 ...
 if (returnCode === 6 && stream?.fd in PHPWASM.child_proc_by_fd) {
  return Asyncify.handleSleep((function(wakeUp) {
   ...
   function poll() {
    ...
    if (returnCode !== 6 || ++retries > maxRetries || !(stream?.fd in PHPWASM.child_proc_by_fd) || PHPWASM.child_proc_by_fd[stream?.fd]?.exited || !PHPWASM.child_proc_by_fd[stream?.fd]?.stdinIsDevice || FS.isClosed(stream)) {
     wakeUp(returnCode);
    } else {
...
var setTempRet0 = function() {
 return (setTempRet0 = Module["asm"]["jb"]).apply(null, arguments);
};

var ___errno_location = function() {
 return (___errno_location = Module["asm"]["kb"]).apply(null, arguments);
};
