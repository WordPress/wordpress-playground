...
export async function getIntlExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(__dirname, '../jspi/extensions/intl/7_2/intl.so');
	} else {
		return join(__dirname, '../asyncify/extensions/intl/7_2/intl.so');
	}
}

export async function getXdebugExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(__dirname, '../jspi/extensions/xdebug/7_2/xdebug.so');
	} else {
		return join(__dirname, '../asyncify/extensions/xdebug/7_2/xdebug.so');
	}
}
