...
var _fd_close = function fd_close(fd) {
  return Asyncify.handleAsync(async () => {
    ...
    if (vfsPathResolutionErrno !== 0) {
      _js_wasm_trace("fd_close(%d) get_vfs_path_from_fd error %d", fd, vfsPathResolutionErrno);
      /*
    ...
    try {
      ...
      await PHPLoader.fileLockManager.releaseLocksForProcessFd(PHPLoader.processId, fd, nativeFilePath);
...
var locking = {
  ...
  check_lock_params(fd, l_type) {
    ...
    if ((l_type === locking.F_WRLCK && accessMode === emscripten_O_RDONLY) || (l_type === locking.F_RDLCK && accessMode === emscripten_O_WRONLY)) {
  ...
  return Asyncify.handleAsync(async () => {
    ...
    /**
      ...
      /*
        ...
        l_whence: HEAP16[// Shift right by 1 to divide by 2^1.
        (flockStructAddress + emscripten_flock_l_whence_offset) >> 1],
        l_start: HEAP64[// Shift right by 3 to divide by 2^3.
    ...
    /**
      ...
      /*
        ...
        (flockStructAddress + emscripten_flock_l_type_offset) >> 1] = fields.l_type;
      }
      if (fields.l_whence !== undefined) {
        HEAP16[// Shift right by 1 to divide by 2^1.
        (flockStructAddress + emscripten_flock_l_whence_offset) >> 1] = fields.l_whence;
      }
      if (fields.l_start !== undefined) {
        HEAP64[// Shift right by 3 to divide by 2^3.
        (flockStructAddress + emscripten_flock_l_start_offset) >> 3] = fields.l_start;
      }
      if (fields.l_len !== undefined) {
        HEAP64[// Shift right by 3 to divide by 2^3.
        (flockStructAddress + emscripten_flock_l_len_offset) >> 3] = fields.l_len;
      }
      if (fields.l_pid !== undefined) {
        HEAP32[// Shift right by 2 to divide by 2^2.
        (flockStructAddress + emscripten_flock_l_pid_offset) >> 2] = fields.l_pid;
      }
    }
    /**
    ...
    switch (cmd) {
     case emscripten_F_GETLK:
      {
        ...
        if (!locking.is_path_to_shared_fs(vfsPath)) {
          _js_wasm_trace("fcntl(%d, F_GETLK) locking is not implemented for non-NodeFS path '%s'", fd, vfsPath);
        ...
        [absoluteStartOffset, errno] = get_base_address(fd, flockStruct.l_whence, flockStruct.l_start);
        if (errno !== 0) {
        ...
        try {
          const nativeFilePath = locking.get_native_path_from_vfs_path(vfsPath);
          const conflictingLock = await Promise.resolve(PHPLoader.fileLockManager.findFirstConflictingByteRangeLock(nativeFilePath, {
          ...
          if (conflictingLock === undefined) {
            _js_wasm_trace("fcntl(%d, F_GETLK) %s findFirstConflictingByteRangeLock type=unlocked start=0x%x end=0x%x", fd, vfsPath, absoluteStartOffset, absoluteStartOffset + flockStruct.l_len);
            update_flock_struct(flockStructAddr, {
              l_type: F_UNLCK
            });
            return 0;
          }
          _js_wasm_trace("fcntl(%d, F_GETLK) %s findFirstConflictingByteRangeLock type=%s start=0x%x end=0x%x conflictingLock %d", fd, vfsPath, conflictingLock.type, conflictingLock.start, conflictingLock.end, conflictingLock.pid);
     ...
     case emscripten_F_SETLK:
      {
        ...
        try {
          ...
          const succeeded = (await Promise.resolve(PHPLoader.fileLockManager.lockFileByteRange(nativeFilePath, rangeLock)));
          _js_wasm_trace("fcntl(%d, F_SETLK) %s lockFileByteRange returned %d for range lock %s", fd, vfsPath, succeeded, rangeLock);
     ...
     // @TODO: Implement a blocking version of F_SETLKW instead of
      ...
      case emscripten_F_SETLKW:
      {
        ...
        [vfsPath, errno] = locking.get_vfs_path_from_fd(fd);
        if (errno !== 0) {
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s get_vfs_path_from_fd errno %d", fd, vfsPath, errno);
          return -errno;
        }
        if (!locking.is_path_to_shared_fs(vfsPath)) {
          _js_wasm_trace("fcntl(%d, F_SETLKW) locking is not implemented for non-NodeFS path %s", fd, vfsPath);
          // If not a NodeFS path, we can't lock it.
          // Default to succeeding as Emscripten does.
          return 0;
        }
        ...
        [absoluteStartOffset, errno] = get_base_address(fd, flockStruct.l_whence, flockStruct.l_start);
        if (errno !== 0) {
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s get_base_address errno %d", fd, vfsPath, errno);
          return -errno;
        }
        if (!(flockStruct.l_type in locking.fcntlToLockState)) {
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s invalid lock type %d", fd, vfsPath, flockStruct.l_type);
          return -ERRNO_CODES.EINVAL;
        }
        errno = locking.check_lock_params(fd, flockStruct.l_type);
        if (errno !== 0) {
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s check_lock_params errno %d", fd, vfsPath, errno);
          return -errno;
        }
        locking.maybeLockedFds.add(fd);
        const requestedLockType = locking.fcntlToLockState[flockStruct.l_type];
        ...
        try {
          const nativeFilePath = locking.get_native_path_from_vfs_path(vfsPath);
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s calling lockFileByteRange for range lock %s", fd, vfsPath, rangeLock);
          const succeeded = (await Promise.resolve(PHPLoader.fileLockManager.lockFileByteRange(nativeFilePath, rangeLock)));
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s lockFileByteRange returned %d for range lock %s", fd, vfsPath, succeeded, rangeLock);
          return succeeded ? 0 : -ERRNO_CODES.EAGAIN;
        } catch (e) {
          _js_wasm_trace("fcntl(%d, F_SETLKW) %s lockFileByteRange error %s for range lock %s", fd, vfsPath, e, rangeLock);
          return -ERRNO_CODES.EINVAL;
        }
      }
     ...
     case emscripten_F_SETFL:
      {
        ...
        stream.flags = (arg & PHPWASM.SETFL_MASK) | (stream.flags & ~PHPWASM.SETFL_MASK);
        return 0;
      }
...
var _js_flock = function js_flock(fd, op) {
  return Asyncify.handleAsync(async () => {
    ...
    try {
      const nativeFilePath = locking.get_native_path_from_vfs_path(vfsPath);
      const obtainedLock = (await Promise.resolve(PHPLoader.fileLockManager.lockWholeFile(nativeFilePath, {
        type: lockOpType,
...
var _js_release_file_locks = function js_release_file_locks() {
  return Asyncify.handleAsync(async () => {
    ...
    try {
      await Promise.resolve(PHPLoader.fileLockManager.releaseLocksForProcess(pid));
...
var _malloc, ___errno_location, _calloc, _wasm_popen, _wasm_php_exec, _php_pollfd_for, _htons, _ntohs, _htonl, _wasm_sleep, _fflush, _flock, _initgroups, _wasm_read, ___wrap_usleep, ___wrap_select, _wasm_set_sapi_name, _wasm_set_phpini_path, _wasm_add_cli_arg, _run_cli, _wasm_add_SERVER_entry, _wasm_add_ENV_entry, _wasm_set_query_string, _wasm_set_path_translated, _wasm_set_skip_shebang, _wasm_set_request_uri, _wasm_set_request_method, _wasm_set_request_host, _wasm_set_content_type, _wasm_set_request_body, _wasm_set_content_length, _wasm_set_cookies, _wasm_set_request_port, _wasm_sapi_request_shutdown, _wasm_sapi_handle_request, _php_wasm_init, _wasm_free, _wasm_get_end_offset, ___wrap_getpid, _wasm_trace, ___funcs_on_exit, ___dl_seterr, __emscripten_find_dylib, _emscripten_builtin_memalign, __emscripten_timeout, _emscripten_get_sbrk_ptr, ___trap, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, memory, __indirect_function_table, wasmTable, wasmMemory;
