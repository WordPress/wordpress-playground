...
export async function getRedisExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(packageDir, 'jspi/extensions/redis/8_3/redis.so');
	}
	throw new Error(
...
export async function getMemcachedExtensionPath(): Promise<string> {
	if (await jspi()) {
		return join(packageDir, 'jspi/extensions/memcached/8_3/memcached.so');
	} else {
		return join(packageDir, 'asyncify/extensions/memcached/8_3/memcached.so');
	}
}
