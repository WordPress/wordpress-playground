...
export class PackedRecordArray {
	...
	setElement(idx, obj) {
		...
		for (let field of this.fields) {
			field.typeArray[idx * field.typeElemSize + field.typeOffset] =
				obj[field.name] || 0;
		}
	}
...
export class ChunkedRecordArray {
	...
	resize(newSize) {
		...
		for (let i = this.chunks.length; i < chunkCount; i++) {
			this.chunks.push(
				new PackedRecordArray(this.fields, this.chunkSize)
			);
		}
	...
	setElement(idx, obj) {
		...
		this.chunks[Math.floor(idx / this.chunkSize)].setElement(
			idx % this.chunkSize,
			obj
		);
	}
	...
	setElementFieldValue(idx, fieldName, fieldValue) {
		this.chunks[Math.floor(idx / this.chunkSize)].setElementFieldValue(
			idx % this.chunkSize,
			fieldName,
			fieldValue
		);
	...
	getElement(idx) {
		return this.chunks[Math.floor(idx / this.chunkSize)].getElement(
			idx % this.chunkSize
		);
	}
	...
	getElementFieldValue(idx, fieldName) {
		return this.chunks[
			Math.floor(idx / this.chunkSize)
		].getElementFieldValue(idx % this.chunkSize, fieldName);
	}
}
