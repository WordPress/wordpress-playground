...
function renderSVGMetricValuesPlot(viewPort, profileData, metric, timeRange) {
	...
	const valueRange = timeComponentMetric
		? new math.Range(0, 1)
		: profileData.getStats().getRange(metric);
			...
			svg.createNode(
				...
				(node) => {
					const formatter = timeComponentMetric
						? fmt.pct
						: profileData.getMetricFormatter(metric);
...
class Widget {
	constructor(container, profileData) {
		...
		this.functionColorResolver = (functionName, defaultColor) => {
			...
			if (this.highlightedFunctionName) {
				color = math.Vec3.createFromHTMLColor(color)
					.mult(
						functionName == this.highlightedFunctionName
							? 1.5
							: 0.33
					)
					.toHTMLColor();
		...
		$(document).on('keyup', '#search_query', (e) => {
			...
			if (key === 'Enter' || key === 13) {
				var eventName = $('#search_query').val()
					? 'spx-search'
					: 'spx-search-clear';
	...
	notifyTimeRangeUpdate(timeRange) {
		this.timeRange = timeRange;
		this.timeRangeStats = this.profileData.getTimeRangeStats(
			this.timeRange
		);

		$(window).trigger('spx-timerange-update', [
			this.timeRange,
			this.timeRangeStats,
		]);
	}
	...
	notifyColorSchemeModeUpdate(colorSchemeMode) {
		this.colorSchemeMode = colorSchemeMode;
		$(window).trigger('spx-colorscheme-mode-update', [
	...
	repaint() {
		...
		this.repaintTimeout = setTimeout(() => {
			...
			if (id === 'flatprofile') {
				initialScrollPos =
					document.querySelector('#flatprofile > div').scrollTop;
			}
...
export class ColorSchemeManager extends Widget {
	static get MODE_DEFAULT() {
		return 'default';
	}
	static get MODE_CATEGORY() {
		return 'category';
	}
	...
	constructor(container, profileData) {
		...
		this.container
			.find('input[name="colorscheme-mode"]:radio')
			.on('change', (e) => {
				if (!e.target.checked) {
					return;
				}
...
<li class="category" data-index=${i}>
	...
	onCategoryEdit(elem) {
		...
		switch (elem.name) {
			...
			case 'colorpicker':
				this.openPicker = idx;
				categories[idx].color = elem.picker.rgb.map((n) =>
					Math.floor(n)
				);
				break;
...
class SVGWidget extends Widget {
	...
	onContainerResize() {
		...
		this.viewPort.resize(this.container.width(), this.container.height());
	}
}
	...
	render() {
		...
		const getCurrentMetricValue = (x) => {
			return this.profileData
				...
				.lerp(Math.pow(x, exp) / Math.pow(this.viewPort.width, exp));
		};
		...
		for (let i = 0; i < this.viewPort.width; i += step * 20) {
			this.viewPort.appendChildToFragment(
				svg.createNode(
					...
					(node) => {
						node.textContent = this.profileData.getMetricFormatter(
							this.currentMetric
						)(getCurrentMetricValue(i));
					}
				)
...
export class OverView extends SVGWidget {
		...
		this.container.on('mousedown mousemove', (e) => {
			if (e.type == 'mousemove' && e.buttons != 1) {
				if (
					math.dist(
						e.clientX,
						this.viewTimeRange.getViewRange().begin
					) < 4
				) {
					...
					math.dist(
						e.clientX,
						this.viewTimeRange.getViewRange().end
					) < 4
				) {
	...
	render() {
		...
		renderSVGTimeGrid(this.viewPort, this.profileData.getTimeRange());
...
export class TimeLine extends SVGWidget {
	constructor(container, profileData) {
		...
		this.container.mouseup((e) => {
			...
			$(window).trigger('spx-highlighted-function-update', [
				callIdx != null
		...
		$(this.viewPort.node).dblclick((e) => {
			...
			this.notifyTimeRangeUpdate(
				this.profileData.getCall(callIdx).getTimeRange()
			);
		});
	...
	_renderCallInfo(callIdx) {
		...
		const currentMetricName = this.profileData.getMetricInfo(
			this.currentMetric
		).name;
		const formatter = this.profileData.getMetricFormatter(
			this.currentMetric
		);
...
export class FlameGraph extends SVGWidget {
	...
	render() {
		...
		const cgRoot = this.timeRangeStats
...
<table width="${this.container.width() - 20}px"><tbody>
		...
		const functionsStats = this.timeRangeStats
			.getFunctionsStats()
			.getValues()
			.filter(
				(stats) =>
...
<tr>
    <td
        ...
        title="${functionLabel}"
        style="text-align: left; font-size: 12px; color: black; background-color: ${this.functionColorResolver(
			stats.functionName,
			getCallMetricValueColor(
				this.profileData,
				this.currentMetric,
    ...
    <td width="80px">${formatter(exc)}</td>
		...
		this.container
			.find('th[data-sort="' + this.sortCol + '"]')
			.addClass('sort');
