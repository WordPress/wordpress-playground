...
class CallListEntry {
	...
	getMetricValue(type, metric) {
		return this.list.array.getElementFieldValue(
			this.idx,
			type + '_' + metric
		);
	}
	...
	getIncMetricValues() {
		return this.getEndMetricValues()
			.copy()
			.sub(this.getStartMetricValues());
	}
	...
	getParent() {
		const parentIdx = this.list.array.getElementFieldValue(
			this.idx,
			'parentIdx'
		);
		if (parentIdx < 0) {
	...
	getDepth() {
		let parentIdx = this.list.array.getElementFieldValue(
			this.idx,
			'parentIdx'
		);
		let depth = 0;
		while (parentIdx >= 0) {
			parentIdx = this.list.array.getElementFieldValue(
				parentIdx,
				'parentIdx'
			);
			depth++;
	...
	getCycleDepth() {
		...
		let cycleDepth = 0;
		while (parentIdx >= 0) {
			const parentFunctionIdx = this.list.array.getElementFieldValue(
				parentIdx,
				'functionIdx'
			);
...
class MetricValuesList {
	...
	getCumCostStats(range) {
		...
		if (
			...
			this.array.getElement(firstIdx)['wt'] < range.begin
		) {
			firstIdx++;
		}
	...
	_findNearestIdx(time, range) {
		...
		if (time < centerTime) {
			return this._findNearestIdx(
				time,
				new math.Range(range.begin, center)
			);
		}
		...
		for (let call of calls) {
			...
			if (!stats) {
				stats = {
					...
					maxCycleDepth: 0,
					called: 0,
					inc: MetricValueSet.createFromMetricsAndValue(
						call.getMetrics(),
						0
					),
					exc: MetricValueSet.createFromMetricsAndValue(
						call.getMetrics(),
						0
					),
				};
	...
	getTimeRangeStats(range, lowerBound, upperBound) {
		...
		for (const child of this.children) {
			timeRangeStats.merge(
				child.getTimeRangeStats(range, lowerBound, upperBound)
			);
			...
			[
				...
				(next) => {
					if (lCallRefs.length == 0) {
						tree.cumCostStats.merge(
							metricValuesList.getCumCostStats(lRange)
						);
						next();
				...
				(next) => {
					if (rCallRefs.length == 0) {
						tree.cumCostStats.merge(
							metricValuesList.getCumCostStats(rRange)
						);
						next();
...
class ProfileData {
	constructor(
		...
		console.log(
			'tree',
			callRangeTree.getMaxDepth(),
			callRangeTree.getNodeCount(),
			callList.getSize()
		);
	...
	getTimeRangeStats(range) {
		...
		const timeRangeStats = this.callRangeTree.getTimeRangeStats(range);
	...
	getCalls(range, minDuration) {
		...
		const callRefs = this.callRangeTree.getCallRefs(range, minDuration);
...
export class ProfileDataBuilder {
	...
	setMetadata(metadata) {
		...
		this.metricValuesList = new MetricValuesList(this.metrics);
