...
export function init() {
	...
	$('div[data-layout-splitter-target]').mousedown((e) => {
		resizedElement = document.getElementById(
			e.target.getAttribute('data-layout-splitter-target')
		);
		dir = parseInt(
			e.target.getAttribute('data-layout-splitter-dir') || '1'
		);
		horizontal =
			'x' === (e.target.getAttribute('data-layout-splitter-axis') || 'x');
		min = Math.max(
			10,
			parseInt(e.target.getAttribute('data-layout-splitter-min'))
		);
		max = Math.round(
			0.9 *
				(horizontal
