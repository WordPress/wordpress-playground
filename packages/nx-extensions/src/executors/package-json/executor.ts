...
export default async function* packageJsonExecutor(
  ...
  for await (const event of startBuild(options, context)) {
    ...
    if (event.success) {
      const built = await buildPackageJson(event, options, context, helperDependencies, monorepoDependencies);
      if (built === false) {
        return {
          success: false,
        }
...
function getMonorepoDependencies(context: ExecutorContext): MonorepoDependency[] {
  const monorepoDeps: MonorepoDependency[] = [];
  for (const repoDep of context.projectGraph.dependencies[context.projectName]) {
    ...
    const targetSourceRoot = context.projectGraph.nodes[repoDep.target].data.root
