...
Developers can preview plugins and themes from a repository with two alternatives: using the resource type `git:directory` or pointing to the zip file from a CI pipeline. For the first approach, we are demonstrating via [JSON Blueprints](/blueprints):
