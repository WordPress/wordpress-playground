...
| `overlay`          |                       | Opens a Playground tool on page load. Supports `new` for the Dock's **New** pane. `blueprints` is kept as a compatibility alias that opens the same **New** pane. For example, `?overlay=blueprints`. The parameter is removed from the URL when the pane is closed.                                                                                                                                                                                                        |
