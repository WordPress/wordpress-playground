...
| `mcp`              |                       | Starts the MCP (Model Context Protocol) server bridge, allowing external MCP clients to connect to and control the Playground instance. Does not accept any values. If `mcp` is added as a URL parameter, the bridge will be started.                                                                                                                                                                                                                                       |
| `mcp-port`         | `7999`                | Sets the WebSocket port used by the MCP bridge to communicate with the MCP server. Must be used together with `mcp`. For example, `mcp&mcp-port=8080`.                                                                                                                                                                                                                                                                                                                      |
