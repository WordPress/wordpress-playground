...
| `plugin`           |                       | Installs the specified plugin. Accepts a WordPress.org slug (e.g. `wp-lazy-loading`), a direct ZIP URL (e.g. `https://example.com/plugin.zip`), or a Git repository URL from GitHub/GitLab (e.g. `https://github.com/user/project`) or any host with a `.git` suffix (e.g. `https://git.example.com/org/repo.git`). You can pre-install multiple plugins by repeating the `plugin` parameter: `plugin=coblocks&plugin=wp-lazy-loading&…`. Installing a plugin automatically logs the user in as an admin. |
