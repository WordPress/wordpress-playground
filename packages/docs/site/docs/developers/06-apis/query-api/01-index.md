...
| `plugin`           |                       | Installs the specified plugin. Accepts a WordPress.org slug (e.g. `wp-lazy-loading`), a ZIP URL (e.g. `https://example.com/plugin.zip`), a GitHub/GitLab repository URL (e.g. `https://github.com/user/project`), or any Git repository URL ending in `.git`. You can pre-install multiple plugins by repeating the `plugin` parameter: `plugin=coblocks&plugin=wp-lazy-loading&…`. Installing a plugin automatically logs the user in as an admin. |
