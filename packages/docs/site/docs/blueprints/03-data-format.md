...
- `php` (string): Loads the specified PHP version. Accepts `7.4`, `8.0`, `8.1`, `8.2`, `8.3`, `8.4`, `8.5`, `latest`, or `master`. Minor versions like `7.4.1` are not supported. Use `master` to preview the next PHP version from php-src `master`; it is currently supported by the web runtime only.
