...
| `importFile`   |               | Imports a site from the specified zip file URL. The file must be served with CORS enabled. The step automatically logs the user in as an admin, unless the default user has been changed or option `login` is set to `0` (zero)                                                                                                                                |
