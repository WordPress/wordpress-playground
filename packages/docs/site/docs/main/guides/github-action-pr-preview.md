...
<strong>Use the action and reusable workflows in different places</strong>

  The direct action, <code>WordPress/action-wp-playground-pr-preview@v3</code>, is a regular GitHub Action. Reference it as a step under <code>jobs.<job_id>.steps[].uses</code>.

The build-step setup uses reusable workflows, <code>preview-build.yml@v3</code> and <code>preview-publish.yml@v3</code>. Reference those at the job level under <code>jobs.<job_id>.uses</code>.
...
<div class="callout callout-info">
  ...
  <strong>Why two workflow files?</strong>
