...
description: Descubra as diferentes maneiras de usar Blueprints, incluindo via fragmento de URL, parâmetro de consulta, pacotes e a API JavaScript.
---
...
-   Usando pacotes Blueprint (arquivos ZIP ou diretórios).
...
A maneira mais fácil de começar a usar Blueprints é colar um no "fragmento" da URL no site WordPress Playground, ex: `https://playground.wordpress.net/#{"preferredVersions...`.
...
:::tip
Em JavaScript, você pode obter uma versão compacta de qualquer JSON blueprint com [`JSON.stringify`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) e [`JSON.parse`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse)
Exemplo:
...
Para executá-lo, vá para https://playground.wordpress.net/#eyIkc2NoZW1hIjogImh0dHBzOi8vcGxheWdyb3VuZC53b3JkcHJlc3MubmV0L2JsdWVwcmludC1zY2hlbWEuanNvbiIsInByZWZlcnJlZFZlcnNpb25zIjogeyJwaHAiOiAiNy40Iiwid3AiOiAiNi41In19
...
:::tip
Em JavaScript, você pode obter qualquer JSON blueprint em [formato Base64](https://developer.mozilla.org/en-US/docs/Glossary/Base64#javascript_support) com a função global `btoa()`.
...
Quando seu Blueprint fica muito pesado, você pode carregá-lo via o parâmetro de consulta `?blueprint-url` na URL, assim:
...
Observe que o Blueprint deve ser publicamente acessível e servido com [o cabeçalho `Access-Control-Allow-Origin` correto](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Origin):
...
O parâmetro `?blueprint-url` agora também suporta pacotes Blueprint em formato ZIP. Um pacote Blueprint é um arquivo ZIP que contém um arquivo `blueprint.json` no nível raiz, junto com quaisquer recursos adicionais referenciados pelo Blueprint.

<!-- For example, you can load a Blueprint bundle like this: -->

Por exemplo, você pode carregar um pacote Blueprint assim:
...
Ao usar um pacote Blueprint, você pode referenciar recursos empacotados usando o tipo de recurso `bundled`:
...
Você também pode usar Blueprints com a API JavaScript usando a função `startPlaygroundWeb()` do pacote `@wp-playground/client`. Aqui está um exemplo pequeno e autocontido que você pode executar no JSFiddle ou CodePen:
