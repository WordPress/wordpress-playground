...
description: Uma referência técnica para "Referências de Recursos." Aprenda como usar arquivos externos para temas, plugins e conteúdo.
---
...
:::info
Passos de Blueprints como [`installPlugin`](/blueprints/steps#InstallPluginStep) ou [`installTheme`](/blueprints/steps#InstallThemeStep) requerem uma localização do plugin ou tema a ser instalado.

Essa localização pode ser definida como [um recurso `URL`](#urlreference) do arquivo `.zip` contendo o tema ou plugin. Também pode ser definida como um recurso [`wordpress.org/plugins`](#corepluginreference) ou [`wordpress.org/themes`](#corethemereference) para aqueles plugins/temas publicados nos diretórios oficiais do WordPress.
:::
...
O recurso `URLReference` é usado para referenciar arquivos que estão armazenados em um servidor remoto. O recurso `URLReference` é definido da seguinte forma:
...
Para usar o recurso `URLReference`, você precisa fornecer a URL do arquivo. Por exemplo, para referenciar um arquivo chamado "index.html" que está armazenado em um servidor remoto, você pode criar um `URLReference` da seguinte forma:
...
O tipo de recurso `url` funciona realmente em combinação com passos de blueprint como [`installPlugin`](/blueprints/steps#InstallPluginStep) ou
[`installTheme`](http://localhost:3000/wordpress-playground/blueprints/steps#InstallThemeStep).
Esses passos requerem um `ResourceType` para definir a localização do plugin ou tema a instalar.
...
Com um `"resource": "url"` podemos definir a localização de um `.zip` contendo o plugin/tema via uma URL que pode apontar diretamente para um repositório GitHub.
...
:::tip
O projeto Playground fornece um [Proxy GitHub](https://playground.wordpress.net/proxy) que permite que você gere um `.zip` de um repositório (ou mesmo uma pasta dentro de um repo) contendo seu plugin ou tema. Esta ferramenta é muito útil para evitar problemas de CORS, entre outros.
:::
...
O recurso `GitDirectoryReference` é usado para referenciar um diretório dentro de um repositório Git. Isso é útil quando um plugin ou tema vive em uma subpasta de um repo, ou quando você quer instalar de um branch, tag ou commit específico.
...
-   Ele lida com buscas com proxy CORS e checkouts esparsos, então você pode usar URLs que apontam para subdiretórios ou branches específicos.
-   Este recurso pode ser usado com passos como [`installPlugin`](/blueprints/steps#InstallPluginStep) e [`installTheme`](/blueprints/steps#InstallThemeStep).
...
O recurso _VFSReference_ é usado para referenciar arquivos que estão armazenados em um sistema de arquivos virtual (VFS). O VFS é um sistema de arquivos que é armazenado na memória e pode ser usado para armazenar arquivos que não fazem parte do sistema de arquivos do sistema operacional. O recurso _VFSReference_ é definido da seguinte forma:
...
Para usar o recurso _VFSReference_, você precisa fornecer o caminho para o arquivo no VFS. Por exemplo, para referenciar um arquivo chamado "index.html" que está armazenado na raiz do VFS, você pode criar um _VFSReference_ da seguinte forma:
...
O recurso _LiteralReference_ é usado para referenciar arquivos que estão armazenados como literais no código. O recurso _LiteralReference_ é definido da seguinte forma:
...
Para usar o recurso _LiteralReference_, você precisa fornecer o nome do arquivo e seu conteúdo. Por exemplo, para referenciar um arquivo chamado "index.html" que contém o texto "Hello, World!", você pode criar um _LiteralReference_ da seguinte forma:
...
O recurso `BundledReference` é usado para referenciar arquivos que são empacotados com o próprio Blueprint. Isso é particularmente útil para criar pacotes Blueprint autocontidos que incluem todos os recursos necessários. O recurso `BundledReference` é definido da seguinte forma:
...
Para usar o recurso `BundledReference`, você precisa fornecer o caminho relativo para o arquivo dentro do pacote. Por exemplo, para referenciar um arquivo chamado "plugin.php" que é empacotado com o Blueprint, você pode criar um `BundledReference` da seguinte forma:
...
Pacotes Blueprint podem ser distribuídos em vários formatos, incluindo:
...
-   Arquivos ZIP com um arquivo `blueprint.json` de nível superior
-   Diretórios contendo um arquivo `blueprint.json` e recursos relacionados
-   URLs remotas onde o Blueprint e seus recursos são hospedados juntos
