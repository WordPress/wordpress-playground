...
WebAssembly PHP ビルドの基盤となる低レベルの制御が必要な場合は、PHP WebAssembly ランタイムを同梱する [`@php-wasm/node` パッケージ](https://npmjs.org/@php-wasm/node) をご覧ください。このパッケージは、Node.js 向けのすべての WordPress Playground ツールの中核を成しています。
