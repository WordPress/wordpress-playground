...
## Playground を使ったローカル WordPress 開発

<!--
## Local WordPress Development with Playground
-->

Playground は、WordPress サイトのセットアップと管理を効率化するためのさまざまな開発環境を提供します。

<!--
Playground offers various development environments to streamline setting up and managing WordPress sites.
-->

すぐに始めるには、https://playground.wordpress.net/ にある [パブリック Playground ウェブインスタンス](/web-instance) をご利用ください。または、[独自の WordPress Playground をホスト](/developers/architecture/host-your-own-playground) することもできます。
...
Playground は、簡単なインストールと使いやすさを重視した、ローカル WordPress 開発用のツールも提供しています。

<!--
Playground also provides tools for local WordPress development, prioritizing easy installation and usability:
-->

- **[@wp-playground/cli](/developers/local-development/wp-playground-cli):** ターミナルから WordPress サイトをすばやく起動するためのコマンドライン ツールです。
- **[Visual Studio Code 拡張機能](/developers/local-development/vscode-extension):** WordPress 開発を VS Code に直接統合し、人気のエディター内でシームレスなエクスペリエンスを実現します。
...
より高度な制御が必要な人のために、Playground は Node.js 用のツールを提供しています。

<!--
For those needing more control, Playground offers tools for Node.js:
-->

- **[Node.js の WordPress Playground](/developers/local-development/php-wasm-node):** WebAssembly PHP ビルドの低レベル制御については、PHP WebAssembly ランタイムが含まれている [`@php-wasm/node` パッケージ](https://npmjs.org/@php-wasm/node) を調べてください。
