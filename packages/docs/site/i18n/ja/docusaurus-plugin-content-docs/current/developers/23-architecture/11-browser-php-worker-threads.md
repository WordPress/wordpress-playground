...
これは面倒な作業になる可能性があるため、Playground では便利な [consumeAPI](/api/web/function/consumeAPI) 関数を提供しています。この関数はメッセージ交換を抽象化し、Web Worker から特定の関数を公開します。そのため、上記の例では `phpClient.run` を呼び出すことができます。
