...
export class MarkdownEmitter {
	...
	protected writeNode(
		...
		switch (docNode.kind) {
			...
			case DocNodeKind.CodeSpan: {
				...
				if (context.insideTable) {
					const code: string = this.getTableEscapedText(
						docCodeSpan.code
					);
			...
			case DocNodeKind.Paragraph: {
				...
				} else if (context.inlineBlockNestingLevel > 0) {
					this.writeNodes(trimmedParagraph.nodes, context);
				} else {
					this.writeNodes(trimmedParagraph.nodes, context);
			...
			case DocNodeKind.SoftBreak: {
				if (!/^\s?$/.test(writer.peekLastCharacter())) {
					...
					const softBreakNode = docNode as DocSoftBreak;
					const child = softBreakNode.getChildNodes()[0];
					let char;
					if (child instanceof DocExcerpt) {
						char = child.content.toString().substring(0, 1);
					}
					writer.write(char || ' ');
				}
				break;
			...
			case DocNodeKind.EscapedText: {
				const docEscapedText: DocEscapedText =
					docNode as DocEscapedText;
			...
			default:
				console.error('Unsupported DocNodeKind kind: ' + docNode.kind);
		}
	}
	...
	protected writePlainText(
		...
		if (middle !== '') {
			...
			let processedMiddle: string = middle;

			// This could be a markdown quote, we don't want to escape that
			const [quoteMaybe] = processedMiddle.match(/^((\s*)>)+/) || [''];
			if (quoteMaybe) {
				// It is a quote, don't escape it
				if (
					writer.peekLastCharacter() === '\n' ||
					writer.peekLastCharacter() === ' ' ||
					writer.peekSecondLastCharacter() === '\n'
				) {
					writer.write(quoteMaybe);
					processedMiddle = processedMiddle.substring(
						quoteMaybe.length
					);
				}
			}
			...
			writer.write(this.getEscapedText(processedMiddle));
