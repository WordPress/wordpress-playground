...
const DEFAULT_MAX_REQUESTS_PER_WORKER: usize = 3_000;
...
struct WorkerPool {
    // The primary stays outside the lazy reaper so the pool always retains one worker. It is not
    // reserved for any request class.
...
struct WorkerPoolState {
    ...
    available_workers: VecDeque<AvailablePhpWorker>,
    created_workers: usize,
    waiting_workers: usize,
    shutdown: bool,
}

struct AvailablePhpWorker {
...
enum WorkerAfterRequest {
    Keep,
    Recycle { delay: Duration },
}

impl WorkerPoolState {
    fn new(available_workers: Vec<Arc<Mutex<PhpWorker>>>, created_workers: usize) -> Self {
        Self {
            primary_available: true,
            available_workers: available_workers
                .into_iter()
                .map(|worker| AvailablePhpWorker {
                    worker,
    ...
    fn select_worker(&mut self, lazy: bool, max_workers: usize) -> WorkerSelection {
        if self.primary_available {
            self.primary_available = false;
            return WorkerSelection::Primary;
        }
        if let Some(worker) = self.available_workers.pop_front() {
            return WorkerSelection::Extra(worker.worker);
        }
        if lazy && self.created_workers < max_workers {
            self.created_workers += 1;
            return WorkerSelection::Spawn;
        }
        WorkerSelection::Wait
    }
}

impl WorkerPoolShared {
    fn new(
        available_workers: Vec<Arc<Mutex<PhpWorker>>>,
        created_workers: usize,
        idle_retirement_delay: Option<Duration>,
    ) -> Arc<Self> {
    ...
    fn release_worker(&self, worker: Arc<Mutex<PhpWorker>>, kind: WorkerKind) {
        let mut state = self
            .state
            .lock()
        ...
        match kind {
            ...
            WorkerKind::Extra => {
                debug_assert!(!state
                    .available_workers
                    .iter()
                    .any(|available| Arc::ptr_eq(&available.worker, &worker)));
                state.available_workers.push_back(AvailablePhpWorker {
                    worker,
                    idle_since: Instant::now(),
                });
            }
    ...
    fn release_spawn_reservation(&self) {
        let mut state = self
            ...
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        state.created_workers = state.created_workers.saturating_sub(1);
        self.worker_available.notify_all();
    }
    ...
    fn start_idle_extra_reaper(shared: &Arc<Self>, delay: Duration) {
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                let Some((position, idle_since)) = state
                    .available_workers
                    .iter()
                    .enumerate()
                    .min_by_key(|(_, available)| available.idle_since)
                    .map(|(position, available)| (position, available.idle_since))
                else {
                    state = shared
                        .worker_available
                        .wait(state)
                ...
                if idle_for < delay {
                    state = shared.wait_timeout(state, delay - idle_for);
                    continue;
                }
                if state.waiting_workers > 0 {
                    shared.worker_available.notify_all();
                    state = shared.wait_timeout(state, delay);
                    continue;
                }

                let can_retire = match state.available_workers[position].worker.try_lock() {
                    Ok(worker) => !worker.recycle_scheduled,
                ...
                let available = state
                    .available_workers
                    .remove(position)
                    .expect("idle worker position remains valid while pool state is locked");
                drop(state);
                shared.finish_idle_extra_retirement(available, release_unused_process_memory);
                state = shared
                    .state
                    .lock()
                    .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn finish_idle_extra_retirement(
        &self,
        available: AvailablePhpWorker,
        reclaim_memory: impl FnOnce(),
    ) {
        drop(available);
        reclaim_memory();
        let mut state = self
    ...
    fn wait_timeout<'a>(
        &self,
        state: std::sync::MutexGuard<'a, WorkerPoolState>,
        duration: Duration,
    ) -> std::sync::MutexGuard<'a, WorkerPoolState> {
        match self.worker_available.wait_timeout(state, duration) {
            Ok((state, _)) => state,
...
impl WorkerPool {
    ...
    fn worker_lease(&self, worker: Arc<Mutex<PhpWorker>>, kind: WorkerKind) -> WorkerLease {
        WorkerLease {
            worker: Some(worker),
    ...
    fn schedule_worker_recycle(&self, worker_lease: WorkerLease, recycle_idle_delay: Duration) {
        let php_artifact = self.php_artifact.clone();
        ...
        let _ = spawn_worker_lease_task(worker_lease, move |worker_lease| {
            ...
            loop {
                ...
                let (mut worker, lock_was_poisoned) =
                    lock_worker_for_recycle(worker_lease.worker());
                if lock_was_poisoned {
                    eprintln!("warning: rebuilding poisoned PHP worker during recycle");
                }
                let idle_for = worker.last_request_finished_at.elapsed();
                if !lock_was_poisoned && idle_for < recycle_idle_delay {
                    sleep_for = worker_recycle_backoff(idle_for, recycle_idle_delay);
                    drop(worker);
                    continue;
                }
                if !lock_was_poisoned && worker.requests_handled < max_requests_per_worker {
                    worker.recycle_scheduled = false;
                    return;
                }
                let start = Instant::now();
...
fn spawn_worker_lease_task<T: Send + 'static>(
    ...
    thread::spawn(move || task(&worker_lease))
}

fn lock_worker_for_recycle(
    worker: &Mutex<PhpWorker>,
) -> (std::sync::MutexGuard<'_, PhpWorker>, bool) {
    match worker.lock() {
...
fn worker_recycle_backoff(idle_for: Duration, recycle_idle_delay: Duration) -> Duration {
    recycle_idle_delay
        .saturating_sub(idle_for)
        .max(Duration::from_millis(1))
}

fn recover_missing_value<T>(
    value: &mut Option<T>,
...
fn discard_instance_on_execution_error<T, U, E>(
    instance: &mut Option<T>,
...
fn max_requests_per_worker_from_env() -> usize {
    std::env::var(MAX_REQUESTS_PER_WORKER_ENV_VAR)
        ...
        .unwrap_or(DEFAULT_MAX_REQUESTS_PER_WORKER)
}
...
fn lazy_worker_idle_timeout_from_env() -> Duration {
    std::env::var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR)
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .filter(|value| *value <= 60_000)
        .map(Duration::from_millis)
        .unwrap_or(DEFAULT_LAZY_WORKER_IDLE_TIMEOUT)
}

fn lazy_worker_idle_retirement_delay(lazy: bool) -> Option<Duration> {
    lazy.then(lazy_worker_idle_timeout_from_env)
}
...
impl Drop for WorkerLease {
    fn drop(&mut self) {
        if let Some(worker) = self.worker.take() {
            self.shared.release_worker(worker, self.kind);
        }
    }
}
...
fn create_worker_pool(
    ...
    for _ in 1..initial_worker_count {
        workers
            .push(runtime.instantiate_php_worker_with_options(php_version, host_options.clone())?);
    }
    Ok(WorkerPool::new(
        workers,
        php_artifact,
...
fn run_listener(
    ...
    host_options.constants.extend(defined_constants_for_host(
    ...
    let mut php =
        runtime.instantiate_php_worker_with_options(&config.options.php, host_options.clone())?;
    ...
    run_startup_steps(
        ...
        actual_port,
        &mut host_options,
        symlink_policy_from_follow(config.options.follow_symlinks),
    )?;
...
fn handle_http_request(
    ...
    match route {
        RouteTarget::Php {
            ...
            // A trapped component instance cannot be entered again. Remove it before the lease
            // returns it to the pool; recover_missing_php will build its replacement on checkout.
            let response =
                discard_instance_on_execution_error(&mut php_worker.php, response_result)?;
            let worker_after_request = worker_pool.mark_request_finished(&mut php_worker)?;
            ...
            if debug {
                eprintln!(
                    "debug: PHP request {} exited {} across {} workers",
                    request_target,
                    response.0,
                    worker_pool.len()
                );
...
fn http_response_from_php_with_counter(
    ...
    let raw_header_bytes = if collect_counter_stats {
        response.headers.iter().map(String::len).sum()
    } else {
        0
    };
    ...
    let mut parsed = parse_php_headers(response.http_status, &response.headers);
...
fn php_response_status(response: &PhpResponse) -> u16 {
    let mut status = response.http_status;
...
pub(crate) fn run_startup_steps(
    ...
    php: &mut PhpInstance,
    port: u16,
    host_options: &mut PhpWorkerOptions,
    symlink_policy: SymlinkPolicy,
) -> Result<()> {
    for (index, step) in steps.iter().enumerate() {
        ...
        {
            merge_defined_constants(&mut host_options.constants, constants);
        ...
        run_startup_step_with_symlink_policy(mounts, php, port, index, step, symlink_policy)?;
    }
    Ok(())
}
...
fn startup_steps_from_blueprint_json_with_context(
    ...
    reject_unsupported_blueprint_top_level_fields(object)?;
...
fn reject_unsupported_blueprint_top_level_fields(
    ...
    for field in [
        ...
        if object.contains_key(field) {
            return Err(CliError::new(format!(
                "Blueprint top-level field `{field}` is not supported by wp-playground-native v1 yet; refusing to ignore it. Use supported startup steps and explicit CLI flags instead."
            )));
        }
    }
    Ok(())
...
fn import_wordpress_files_root(staging_root: &Path, path_in_zip: &str) -> Result<PathBuf> {
    ...
    find_imported_wordpress_root(import_root)
}

const IMPORT_WORDPRESS_ROOT_MARKERS: [&str; 5] = [
    "wp-content",
    "wp-admin",
    "wp-includes",
    "wp-config.php",
...
fn find_imported_wordpress_root(import_root: PathBuf) -> Result<PathBuf> {
    if has_imported_wordpress_root_marker(&import_root) {
    ...
    let mut candidates = entries
        ...
        .filter(|path| path.is_dir() && has_imported_wordpress_root_marker(path));
    let Some(nested_root) = candidates.next() else {
        return Ok(import_root);
    };
...
fn has_imported_wordpress_root_marker(path: &Path) -> bool {
    IMPORT_WORDPRESS_ROOT_MARKERS
        .iter()
...
fn effective_site_url_from_host_options(host_options: &PhpWorkerOptions, port: u16) -> String {
    host_options
        .constants
        .iter()
        .rev()
...
fn parse_php_headers(status: u16, raw_headers: &[String]) -> HttpResponse {
    let mut headers = Vec::new();
...
mod tests {
    ...
    fn host_options_for_test_mounts(mounts: &[Mount], site_url: &str) -> PhpWorkerOptions {
        let mut host_options = PhpWorkerOptions {
            mounts: mounts.to_vec(),
            ..PhpWorkerOptions::default()
        };
        host_options.constants.push((
    ...
    #[test]
    fn parses_typed_php_headers() {
        let response = parse_php_headers(404, &["X-Test: yes".to_string()]);
    ...
    fn server_banner_and_config_summarizes_node_cli_fields() {
        ...
        assert!(summary.contains("PHP 8.2  WordPress 6.9"));
    ...
    #[test]
    fn lazy_worker_idle_timeout_env_override_accepts_milliseconds() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous = std::env::var_os(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR);
        std::env::remove_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR);
        assert_eq!(
            lazy_worker_idle_timeout_from_env(),
            DEFAULT_LAZY_WORKER_IDLE_TIMEOUT
        );

        std::env::set_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR, "250");
        assert_eq!(
            lazy_worker_idle_timeout_from_env(),
            Duration::from_millis(250)
        );

        std::env::set_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR, "not-a-number");
        assert_eq!(
            lazy_worker_idle_timeout_from_env(),
            DEFAULT_LAZY_WORKER_IDLE_TIMEOUT
        );

        std::env::set_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR, "60001");
        assert_eq!(
            lazy_worker_idle_timeout_from_env(),
            DEFAULT_LAZY_WORKER_IDLE_TIMEOUT
        );
        ...
        if let Some(previous) = previous {
            std::env::set_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR, previous);
        } else {
            std::env::remove_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR);
        }
    }
    ...
    #[test]
    fn lazy_worker_retirement_uses_its_own_timeout_and_skips_fixed_pools() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_lazy = std::env::var_os(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR);
        let previous_recycle = std::env::var_os(WORKER_RECYCLE_IDLE_DELAY_ENV_VAR);
        std::env::set_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR, "250");
        std::env::set_var(WORKER_RECYCLE_IDLE_DELAY_ENV_VAR, "5");

        assert_eq!(
            lazy_worker_idle_retirement_delay(true),
            Some(Duration::from_millis(250))
        );
        assert_eq!(lazy_worker_idle_retirement_delay(false), None);
        assert_eq!(
            worker_recycle_idle_delay_from_env(),
            Duration::from_millis(5)
        );

        if let Some(previous) = previous_lazy {
            std::env::set_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR, previous);
        } else {
            std::env::remove_var(LAZY_WORKER_IDLE_TIMEOUT_ENV_VAR);
        }
        if let Some(previous) = previous_recycle {
            std::env::set_var(WORKER_RECYCLE_IDLE_DELAY_ENV_VAR, previous);
        } else {
            std::env::remove_var(WORKER_RECYCLE_IDLE_DELAY_ENV_VAR);
        }
    ...
    #[test]
    fn worker_recycle_backoff_never_busy_spins() {
        assert_eq!(
            worker_recycle_backoff(Duration::ZERO, Duration::ZERO),
            Duration::from_millis(1)
        );
        assert_eq!(
            worker_recycle_backoff(Duration::from_millis(10), Duration::from_millis(50)),
            Duration::from_millis(40)
        );
        assert_eq!(
            worker_recycle_backoff(Duration::from_millis(60), Duration::from_millis(50)),
            Duration::from_millis(1)
        );
    }

    #[test]
    fn missing_worker_recovery_attempts_once_and_can_succeed_later() {
        let attempts = std::cell::Cell::new(0);
        let mut value: Option<u8> = None;

        let first = super::recover_missing_value(&mut value, || {
            attempts.set(attempts.get() + 1);
            Err(crate::CliError::new("transient worker creation failure"))
        });
        assert!(first.is_err());
        assert_eq!(attempts.get(), 1);
        assert_eq!(value, None);

        assert!(super::recover_missing_value(&mut value, || {
            attempts.set(attempts.get() + 1);
            Ok(7)
        })
        .unwrap());
    ...
    #[test]
    fn component_execution_errors_discard_instance_but_php_exit_codes_do_not() {
        let mut component = Some("healthy component");
        let php_fatal =
            discard_instance_on_execution_error(&mut component, Ok::<_, &'static str>(255));
        assert_eq!(php_fatal, Ok(255));
        assert_eq!(component, Some("healthy component"));

        let guest_trap =
            discard_instance_on_execution_error(&mut component, Err::<i32, _>("guest trap"));
        assert_eq!(guest_trap, Err("guest trap"));
    ...
    fn concurrent_requests_select_distinct_workers_without_authentication_reservation() {
        ...
        assert!(matches!(
            state.select_worker(false, 2),
            WorkerSelection::Primary
        ));
        assert!(!state.primary_available);
        match state.select_worker(false, 2) {
            WorkerSelection::Extra(selected) => {
                assert!(Arc::ptr_eq(&selected, &extra_worker));
            }
    ...
    fn released_lazy_worker_satisfies_queued_demand_before_idle_retirement() {
        ...
        let waiter = thread::spawn(move || {
            let mut state = waiter_shared.state.lock().unwrap();
            state.waiting_workers += 1;
            waiting_tx.send(()).unwrap();
            while state.available_workers.is_empty() {
                state = waiter_shared.worker_available.wait(state).unwrap();
            }
            state.waiting_workers -= 1;
            let acquired = state.available_workers.pop_front().unwrap().worker;
            acquired_tx.send(acquired).unwrap();
        });

        waiting_rx.recv_timeout(Duration::from_secs(1)).unwrap();
        shared.release_worker(Arc::clone(&worker), WorkerKind::Extra);
        let acquired = acquired_rx.recv_timeout(Duration::from_secs(1)).unwrap();
        assert!(Arc::ptr_eq(&worker, &acquired));
        waiter.join().unwrap();
        thread::sleep(Duration::from_millis(30));
        assert_eq!(shared.state.lock().unwrap().created_workers, 2);
        shared.shutdown();
    }

    #[test]
    fn unused_lazy_worker_retires_after_idle_grace() {
        let shared = WorkerPoolShared::new(Vec::new(), 2, Some(Duration::from_millis(10)));
        let worker = Arc::new(Mutex::new(retained_worker(0, false)));
        shared.release_worker(worker, WorkerKind::Extra);

        let deadline = Instant::now() + Duration::from_secs(1);
        loop {
            let state = shared.state.lock().unwrap();
            if state.created_workers == 1 {
                assert!(state.available_workers.is_empty());
                break;
            }
            assert!(Instant::now() < deadline, "idle worker did not retire");
            drop(state);
    ...
    #[test]
    fn idle_worker_retirement_reclaims_before_releasing_capacity() {
        let shared = WorkerPoolShared::new(Vec::new(), 2, None);
        let worker = Arc::new(Mutex::new(retained_worker(0, false)));
        let weak_worker = Arc::downgrade(&worker);
        let available = super::AvailablePhpWorker {
            worker,
            idle_since: Instant::now(),
        };

        shared.finish_idle_extra_retirement(available, || {
    ...
    #[test]
    fn withheld_extra_worker_wakes_waiter_only_after_recycle_task_finishes() {
        let shared = WorkerPoolShared::new(Vec::new(), 2, None);
        shared.state.lock().unwrap().primary_available = false;
        ...
        let lease = WorkerLease {
            ...
            kind: WorkerKind::Extra,
        };
        let (task_started_tx, task_started_rx) = mpsc::channel();
        let (finish_task_tx, finish_task_rx) = mpsc::channel();
        let task = spawn_worker_lease_task(lease, move |_| {
            task_started_tx.send(()).unwrap();
            finish_task_rx.recv().unwrap();
        });
        task_started_rx
        ...
        let waiter = thread::spawn(move || {
            ...
            state.waiting_workers -= 1;
            acquired_tx
                .send(state.available_workers.pop_front().unwrap().worker)
                .unwrap();
        });
        waiter_ready_rx
            .recv_timeout(Duration::from_secs(1))
        ...
        assert!(acquired_rx.recv_timeout(Duration::from_millis(25)).is_err());
        {
            ...
            assert!(state.available_workers.is_empty());
            assert_eq!(state.waiting_workers, 1);
        }

        finish_task_tx.send(()).unwrap();
        task.join().unwrap();
        let acquired = acquired_rx.recv_timeout(Duration::from_secs(1)).unwrap();
        assert!(Arc::ptr_eq(&worker, &acquired));
        waiter.join().unwrap();
    ...
    #[test]
    fn withheld_primary_worker_releases_after_recycle_task_error() {
        ...
        let lease = WorkerLease {
            worker: Some(worker),
            shared: Arc::clone(&shared),
            kind: WorkerKind::Primary,
        };

        let task = spawn_worker_lease_task(lease, |_| {
            Err::<(), &'static str>("injected recycle failure")
        });
        assert_eq!(
            task.join().unwrap(),
            Err::<(), &'static str>("injected recycle failure")
    ...
    #[test]
    fn poisoned_worker_lock_can_be_repaired_before_release() {
        let worker = Arc::new(Mutex::new(retained_worker(400, true)));
        let poison_worker = Arc::clone(&worker);
        assert!(thread::spawn(move || {
            let _worker = poison_worker.lock().unwrap();
            panic!("poison worker for recycle recovery test");
        })
        .join()
        .is_err());

        let (mut repaired, was_poisoned) = lock_worker_for_recycle(&worker);
        assert!(was_poisoned);
        repaired.php = None;
        repaired.requests_handled = 0;
        repaired.recycle_scheduled = false;
        drop(repaired);
        worker.clear_poison();

        assert!(!worker.is_poisoned());
        let repaired = worker.lock().unwrap();
    ...
    #[test]
    fn mark_worker_request_finished_keeps_and_recycles() {
        let mut worker = retained_worker(0, false);
        let action =
            mark_worker_request_finished(&mut worker, 2, Duration::from_millis(25)).unwrap();
        ...
        assert!(matches!(
            action,
            WorkerAfterRequest::Recycle { delay } if delay == Duration::from_millis(25)
        ));
    ...
    fn mark_worker_request_finished_keeps_already_scheduled_recycle() {
        ...
        let action =
            mark_worker_request_finished(&mut worker, 1, Duration::from_millis(25)).unwrap();
    ...
    #[test]
    fn native_import_wordpress_files_unwraps_single_parent_directory() {
        let root = temp_dir("import-wordpress-files-wrapped");
        let wordpress_root = root.join("wordpress");
        let tmp_root = root.join("tmp");
        fs::create_dir_all(&wordpress_root).unwrap();
        fs::create_dir_all(&tmp_root).unwrap();
        ...
        let zip = blueprint_zip(vec![
            (
                "playground-export/wp-content/plugins/nested-plugin/plugin.php",
                "<?php // nested plugin",
            ),
            (".DS_Store", "ignored root noise"),
            ("notes/readme.txt", "ignored non-WordPress directory"),
        ]);
        ...
        assert!(wordpress_root
            .join("wp-content/plugins/nested-plugin/plugin.php")
            .is_file());
        assert!(!wordpress_root.join("playground-export").exists());

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn native_import_wordpress_files_keeps_ambiguous_parent_directories() {
        let root = temp_dir("import-wordpress-files-ambiguous-wrappers");
        let wordpress_root = root.join("wordpress");
        let tmp_root = root.join("tmp");
        fs::create_dir_all(&wordpress_root).unwrap();
        fs::create_dir_all(&tmp_root).unwrap();
        ...
        let zip = blueprint_zip(vec![
            ("first/wp-content/plugins/first.php", "<?php // first"),
            ("second/wp-content/plugins/second.php", "<?php // second"),
        ]);
        ...
        assert!(wordpress_root
            .join("first/wp-content/plugins/first.php")
            .is_file());
        assert!(wordpress_root
            .join("second/wp-content/plugins/second.php")
            .is_file());
        assert!(!wordpress_root.join("wp-content").exists());

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn native_import_wordpress_files_unwraps_config_only_parent_directory() {
        let root = temp_dir("import-wordpress-files-wrapped-config");
        let wordpress_root = root.join("wordpress");
        let tmp_root = root.join("tmp");
        fs::create_dir_all(&wordpress_root).unwrap();
        fs::create_dir_all(&tmp_root).unwrap();
        ...
        let zip = blueprint_zip(vec![(
            "playground-export/wp-config-sample.php",
            "<?php // imported sample",
        )]);
        ...
        assert_eq!(
            fs::read_to_string(wordpress_root.join("wp-config-sample.php")).unwrap(),
            "<?php // imported sample"
        );
        assert!(!wordpress_root.join("playground-export").exists());
    ...
    #[test]
    fn rejects_unsupported_behavioral_blueprint_top_level_fields() {
        for (field, value) in [
            ("landingPage", r#""/wp-admin/""#),
            ("preferredVersions", r#"{"php":"8.4","wp":"latest"}"#),
            ("features", r#"{"networking":true}"#),
            ("constants", r#"{"WP_DEBUG":true}"#),
            ("plugins", r#"["hello-dolly"]"#),
        ] {
            let blueprint = format!(r#"{{ "{field}": {value}, "steps": [] }}"#);
            let error = startup_steps_from_blueprint_json(&blueprint, false).unwrap_err();
            assert!(error.to_string().contains(field));
            assert!(error.to_string().contains("refusing to ignore"));
        }
    }
    ...
    fn real_run_php_with_options_startup_step_observes_request_state() {
        ...
        let mut php = runtime
            .instantiate_php_worker_with_options("8.2", host_options)
            .unwrap();
...
"#,
    ...
    fn real_run_wp_installation_wizard_startup_step_installs_wordpress() {
        ...
        let mut php = runtime
            .instantiate_php_worker_with_options(&options.php, host_options)
            .unwrap();
    ...
    fn real_import_wxr_startup_step_imports_fixture_post() {
        ...
        host_options
            ...
            .push(("WP_SITEURL".to_string(), PhpConstantValue::string(site_url)));
        let mut php = runtime
            .instantiate_php_worker_with_options(&options.php, host_options.clone())
            .unwrap();
        ...
        run_startup_steps(
            ...
            port,
            &mut host_options,
            super::SymlinkPolicy::BlockEscapes,
        )
        .unwrap();
...
"#,
    ...
    fn real_import_wordpress_files_startup_step_rewrites_scoped_urls() {
        ...
        let mut source_php = runtime
            .instantiate_php_worker_with_options(&options.php, source_host_options)
            .unwrap();
...
"#,
        ...
        let mut target_php = runtime
            .instantiate_php_worker_with_options(&options.php, target_host_options.clone())
            .unwrap();
