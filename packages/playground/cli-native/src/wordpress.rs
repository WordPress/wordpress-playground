...
pub fn prepare_wordpress(
    ...
    match options.wordpress_install_mode {
        WordPressInstallMode::DownloadAndInstall => {
            let archive_path = wordpress_archive(repo_root, &options.wp)?;
            extract_wordpress_with_static_overlay_without_overwriting(
                repo_root,
                &archive_path,
                &document_root,
            )?;
...
fn wordpress_archive(repo_root: &Path, wp: &str) -> Result<PathBuf> {
    if let Some(path) = bundled_wordpress_archive_if_available(repo_root, wp)? {
        return Ok(path);
    }
...
fn bundled_wordpress_archive_if_available(repo_root: &Path, wp: &str) -> Result<Option<PathBuf>> {
    ...
    let directory = repo_root.join(WORDPRESS_BUILDS_DIR);
    for filename in [format!("{stem}.tar.zst"), format!("{stem}.zip")] {
        let path = directory.join(filename);
        if path.is_file() {
            validate_wordpress_archive(&path)?;
            return Ok(Some(path));
        }
    }
    Ok(None)
...
fn bundled_wordpress_static_zip_for_archive(
    repo_root: &Path,
    archive_path: &Path,
) -> Result<Option<PathBuf>> {
    let bundled_archive_dir = repo_root.join(WORDPRESS_BUILDS_DIR);
    if archive_path.parent() != Some(bundled_archive_dir.as_path()) {
        return Ok(None);
    }
    let Some(filename) = archive_path.file_name().and_then(|name| name.to_str()) else {
        return Ok(None);
    };
    let Some(version) = bundled_wordpress_version_from_filename(filename) else {
        return Ok(None);
    };
    let static_zip = repo_root
        .join(WORDPRESS_STATIC_BUILDS_DIR)
        .join(format!("wp-{version}"))
        .join(WORDPRESS_STATIC_ZIP);
    if static_zip.is_file() {
        return Ok(Some(static_zip));
    }
    if is_tar_zst_path(archive_path) {
        return Err(CliError::new(format!(
            "Bundled WordPress {version} static ZIP is missing: {}",
            static_zip.display()
        )));
    }
    Ok(None)
...
fn latest_bundled_wordpress_version(repo_root: &Path) -> Result<String> {
    ...
    let mut versions = fs::read_dir(&dir)?
        ...
        .filter_map(|path| {
            ...
            let version = bundled_wordpress_version_from_filename(name)?;
    ...
    versions.pop().ok_or_else(|| {
        CliError::new(format!(
            "No bundled WordPress archives found in {}",
            dir.display()
        ))
    })
}
...
fn bundled_wordpress_version_from_filename(filename: &str) -> Option<&str> {
    let name = filename.strip_prefix("wp-")?;
    name.strip_suffix(".tar.zst")
        .or_else(|| name.strip_suffix(".zip"))
}

fn extract_wordpress_archive_without_overwriting(
    archive_path: &Path,
    document_root: &Path,
) -> Result<()> {
    if is_tar_zst_path(archive_path) {
        extract_wordpress_tar_zst_without_overwriting(archive_path, document_root)
    } else {
        extract_wordpress_zip_without_overwriting(archive_path, document_root)
    }
}

fn extract_wordpress_with_static_overlay_without_overwriting(
    repo_root: &Path,
    archive_path: &Path,
    document_root: &Path,
) -> Result<()> {
    let static_zip = bundled_wordpress_static_zip_for_archive(repo_root, archive_path)?;
    if let Some(static_zip) = &static_zip {
        validate_wordpress_static_zip(static_zip)?;
    }
    extract_wordpress_archive_without_overwriting(archive_path, document_root)?;
    if let Some(static_zip) = static_zip {
        extract_validated_wordpress_static_zip_without_overwriting(&static_zip, document_root)?;
    }
    Ok(())
}

#[cfg(test)]
fn extract_wordpress_static_zip_without_overwriting(
    zip_path: &Path,
    document_root: &Path,
) -> Result<()> {
    validate_wordpress_static_zip(zip_path)?;
    extract_validated_wordpress_static_zip_without_overwriting(zip_path, document_root)
}

fn extract_validated_wordpress_static_zip_without_overwriting(
    zip_path: &Path,
    document_root: &Path,
) -> Result<()> {
    ...
    let mut archive = ZipArchive::new(file).map_err(|error| {
        CliError::new(format!(
            "Failed to read WordPress static ZIP {}: {error}",
            zip_path.display()
        ))
    })?;
    for index in 0..archive.len() {
        let mut entry = archive.by_index(index).map_err(|error| {
            CliError::new(format!(
                "Failed to read WordPress static ZIP entry {index}: {error}"
            ))
        })?;
        let relative = safe_static_zip_entry_path(&entry, zip_path)?;
        if relative.as_os_str().is_empty() {
            continue;
        }
        if !create_safe_overlay_parents(document_root, &relative, zip_path)? {
            continue;
        }
...
pub(crate) fn validate_wordpress_static_zip(zip_path: &Path) -> Result<()> {
    let file = fs::File::open(zip_path).map_err(|error| {
        CliError::new(format!(
            "Failed to open WordPress static ZIP {}: {error}",
            zip_path.display()
        ))
    })?;
    let mut archive = ZipArchive::new(file).map_err(|error| {
    ...
    for index in 0..archive.len() {
        let mut entry = archive.by_index(index).map_err(|error| {
            CliError::new(format!(
                "WordPress static ZIP {} has unreadable entry {index}: {error}",
                zip_path.display()
            ))
        })?;
        safe_static_zip_entry_path(&entry, zip_path)?;
        if entry.is_dir() {
            continue;
        }
        has_file = true;
        io::copy(&mut entry, &mut io::sink()).map_err(|error| {
            CliError::new(format!(
                "WordPress static ZIP {} has unreadable contents in entry {}: {error}",
                zip_path.display(),
                entry.name()
            ))
        })?;
    ...
    if !has_file {
        return Err(CliError::new(format!(
            "WordPress static ZIP {} does not contain any files",
            zip_path.display()
        )));
    }
    Ok(())
...
fn safe_static_zip_entry_path<R: io::Read>(
    ...
    {
        return Err(CliError::new(format!(
            "WordPress static ZIP {} contains unsafe entry {}",
            zip_path.display(),
            entry.name()
        )));
    }
...
fn create_safe_overlay_parents(
    document_root: &Path,
    relative: &Path,
    zip_path: &Path,
) -> Result<bool> {
    ...
    for component in parent.components() {
        let Component::Normal(segment) = component else {
            return Err(CliError::new(format!(
                "WordPress static ZIP {} contains unsafe entry path {}",
                zip_path.display(),
                relative.display()
            )));
        };
        ...
        match fs::symlink_metadata(&current) {
            Ok(metadata) if metadata.file_type().is_symlink() => {
                return Err(CliError::new(format!(
                    "Refusing to extract WordPress static ZIP {} through symlink {}",
                    zip_path.display(),
                    current.display()
                )))
            }
...
fn extract_wordpress_tar_zst_without_overwriting(
    archive_path: &Path,
    document_root: &Path,
) -> Result<()> {
    validate_wordpress_tar_zst(archive_path)?;
    let file = fs::File::open(archive_path)?;
    let decoder = zstd::stream::read::Decoder::new(file).map_err(|error| {
        CliError::new(format!(
            "Failed to decompress WordPress tar.zst {}: {error}",
    ...
    let entries = archive.entries().map_err(|error| {
        CliError::new(format!(
            "Failed to read WordPress tar.zst {}: {error}",
            archive_path.display()
        ))
    })?;
    for entry in entries {
        let mut entry = entry.map_err(|error| {
            CliError::new(format!(
                "Failed to read WordPress tar entry in {}: {error}",
                archive_path.display()
            ))
        })?;
        let relative = entry
            ...
            .map_err(|error| {
                CliError::new(format!(
                    "Failed to read WordPress tar entry path in {}: {error}",
                    archive_path.display()
                ))
            })?
            .into_owned();
        validate_tar_entry_path(&relative, archive_path)?;
        if relative.as_os_str().is_empty() {
            continue;
        }
        let target = document_root.join(&relative);
        ...
        if !entry_type.is_file() {
            return Err(CliError::new(format!(
                "WordPress tar.zst {} contains unsupported entry type at {}",
                archive_path.display(),
                relative.display()
            )));
        }
...
pub(crate) fn validate_wordpress_archive(archive_path: &Path) -> Result<()> {
    if is_tar_zst_path(archive_path) {
        validate_wordpress_tar_zst(archive_path)
    } else {
        validate_wordpress_zip(archive_path)
    }
}

fn validate_wordpress_tar_zst(archive_path: &Path) -> Result<()> {
    let file = fs::File::open(archive_path).map_err(|error| {
        CliError::new(format!(
            "Failed to open WordPress tar.zst {}: {error}",
            archive_path.display()
        ))
    })?;
    let decoder = zstd::stream::read::Decoder::new(file).map_err(|error| {
    ...
    for entry in entries {
        let mut entry = entry.map_err(|error| {
            CliError::new(format!(
                "WordPress tar.zst {} has an unreadable entry: {error}",
                archive_path.display()
            ))
        })?;
        let path = entry
            ...
            .map_err(|error| {
                CliError::new(format!(
                    "WordPress tar.zst {} has an unreadable entry path: {error}",
                    archive_path.display()
                ))
            })?
            .into_owned();
        validate_tar_entry_path(&path, archive_path)?;
        let entry_type = entry.header().entry_type();
        ...
        for (required_path, found) in &mut required {
            if entry_type.is_file() && path.as_path() == Path::new(required_path) {
                *found = true;
            }
        }
        let wp_content_relative = path.strip_prefix("wp-content").ok();
        if (path == Path::new("wp-content") && entry_type.is_dir())
            || wp_content_relative.is_some_and(|relative| !relative.as_os_str().is_empty())
        {
            has_wp_content = true;
        }
        if entry_type.is_file() {
            io::copy(&mut entry, &mut io::sink()).map_err(|error| {
                CliError::new(format!(
                    "WordPress tar.zst {} has unreadable contents in entry {}: {error}",
                    archive_path.display(),
                    path.display()
                ))
            })?;
    ...
    for (required_path, found) in required {
        if !found {
            return Err(CliError::new(format!(
                "WordPress tar.zst {} is missing required entry {required_path}",
                archive_path.display()
            )));
        }
    }
    if !has_wp_content {
        return Err(CliError::new(format!(
            "WordPress tar.zst {} is missing required entry wp-content/",
            archive_path.display()
        )));
    }
    Ok(())
...
fn validate_tar_entry_path(path: &Path, archive_path: &Path) -> Result<()> {
    ...
    {
        return Err(CliError::new(format!(
            "WordPress tar.zst {} contains an unsafe entry path: {}",
            archive_path.display(),
            path.display()
        )));
    }
...
mod tests {
    ...
    use super::{
        bundled_version_prefix, bundled_wordpress_static_zip_for_archive, civil_from_unix_days,
        contains_wordpress_files, defined_constants_for_host, ensure_sqlite_database_scaffold,
        ensure_sqlite_integration, ensure_wp_config, extract_wordpress_archive_without_overwriting,
        extract_wordpress_static_zip_without_overwriting,
        extract_wordpress_with_static_overlay_without_overwriting,
        latest_bundled_wordpress_version, normalize_inferred_release_version,
        resolve_wordpress_release_from_offers, validate_sqlite_zip, validate_tar_entry_path,
        validate_wordpress_archive, validate_wordpress_static_zip, version_matches_patch_zero,
        wordpress_archive, VersionOffer, WORDPRESS_BUILDS_DIR, WORDPRESS_STATIC_BUILDS_DIR,
        WORDPRESS_STATIC_ZIP,
    };
    use crate::{
    ...
    fn write_tar_zst_symlink(path: &std::path::Path) {
        let file = fs::File::create(path).unwrap();
        let encoder = zstd::stream::write::Encoder::new(file, 0).unwrap();
        let mut archive = tar::Builder::new(encoder);
        let mut header = tar::Header::new_gnu();
        ...
        header.set_link_name("../escape.php").unwrap();
        header.set_cksum();
        archive
            .append_data(&mut header, "link.php", io::empty())
            .unwrap();
        let encoder = archive.into_inner().unwrap();
        encoder.finish().unwrap();
    }
    ...
    #[test]
    fn maps_patch_versions_to_bundled_minor_archive() {
    ...
    #[test]
    fn resolves_static_zip_from_the_selected_bundled_archive_version() {
        let repo_root = temp_dir("static-version-resolution");
        let archive_dir = repo_root.join(WORDPRESS_BUILDS_DIR);
        fs::create_dir_all(&archive_dir).unwrap();
        let archive = archive_dir.join("wp-6.9.tar.zst");
        fs::write(&archive, b"dynamic").unwrap();
        let static_zip = repo_root
            .join(WORDPRESS_STATIC_BUILDS_DIR)
            .join("wp-6.9")
            .join(WORDPRESS_STATIC_ZIP);
        fs::create_dir_all(static_zip.parent().unwrap()).unwrap();
        fs::write(&static_zip, b"static").unwrap();

        assert_eq!(
            bundled_wordpress_static_zip_for_archive(&repo_root, &archive).unwrap(),
            Some(static_zip)
        );
        assert!(bundled_wordpress_static_zip_for_archive(
            &repo_root,
            &repo_root.join("downloads/wp-6.9.tar.zst")
        )
        .unwrap()
        .is_none());

        let missing = archive_dir.join("wp-7.0.tar.zst");
        let error = bundled_wordpress_static_zip_for_archive(&repo_root, &missing)
            .unwrap_err()
            .to_string();
        let missing_static_zip = repo_root
            .join(WORDPRESS_STATIC_BUILDS_DIR)
            .join("wp-7.0")
            .join(WORDPRESS_STATIC_ZIP);
        assert!(
            error.contains(&missing_static_zip.display().to_string()),
            "{error}"
        );
        let full_zip = archive_dir.join("wp-7.0.zip");
        assert!(
            bundled_wordpress_static_zip_for_archive(&repo_root, &full_zip)
                .unwrap()
                .is_none()
        );
    ...
    #[test]
    fn overlays_static_zip_without_overwriting_dynamic_or_user_files() {
        let repo_root = temp_dir("static-overlay");
        let archive = repo_root.join(WORDPRESS_BUILDS_DIR).join("wp-6.9.tar.zst");
        ...
        write_tar_zst(
            ...
            &[
                ...
                ("wp-load.php", b"<?php"),
                ("wp-admin/install.php", b"dynamic-install"),
                ("wp-includes/version.php", b"<?php"),
                ("wp-content/index.php", b"<?php"),
                ("index.php", b"dynamic-index"),
            ],
        );
        let static_zip = repo_root
            .join(WORDPRESS_STATIC_BUILDS_DIR)
        ...
        write_zip(
            &static_zip,
            &[
                ("wp-admin/css/site-editor.css", b"static-css"),
                ("wp-includes/js/dist/edit-site.js", b"static-js"),
                ("wp-content/themes/test/style.css", b"theme-css"),
                ("index.php", b"static-index"),
            ],
        );
        let destination = repo_root.join("wordpress");
        fs::create_dir_all(destination.join("wp-admin/css")).unwrap();
        fs::write(
            destination.join("wp-admin/css/site-editor.css"),
            b"user-css",
        )
        .unwrap();
        ...
        assert_eq!(
            fs::read(destination.join("wp-admin/css/site-editor.css")).unwrap(),
            b"user-css"
        );
        assert_eq!(
            fs::read(destination.join("wp-includes/js/dist/edit-site.js")).unwrap(),
            b"static-js"
        );
        assert_eq!(
            fs::read(destination.join("wp-content/themes/test/style.css")).unwrap(),
            b"theme-css"
        );
        assert_eq!(
            fs::read(destination.join("index.php")).unwrap(),
            b"dynamic-index"
        );
    ...
    #[test]
    fn rejects_static_zip_traversal_and_symlink_parents() {
        let dir = temp_dir("static-traversal");
        let destination = dir.join("wordpress");
        for (label, entry) in [
            ("parent", "../escape.css"),
            ("backslash", "wp-admin\\..\\escape.css"),
            ("drive", "C:/escape.css"),
        ] {
            let unsafe_zip = dir.join(format!("unsafe-{label}.zip"));
            write_zip(&unsafe_zip, &[(entry, b"escape")]);
            assert!(validate_wordpress_static_zip(&unsafe_zip).is_err());
            assert!(
                extract_wordpress_static_zip_without_overwriting(&unsafe_zip, &destination)
                    .is_err()
            );
        }
        assert!(!dir.join("escape.css").exists());

        #[cfg(unix)]
        {
            use std::os::unix::fs::symlink;

            let safe_zip = dir.join("safe-static.zip");
            write_zip(&safe_zip, &[("wp-admin/css/site-editor.css", b"css")]);
            let outside = dir.join("outside");
            fs::create_dir_all(&outside).unwrap();
            fs::create_dir_all(&destination).unwrap();
            symlink(&outside, destination.join("wp-admin")).unwrap();

            let error = extract_wordpress_static_zip_without_overwriting(&safe_zip, &destination)
                .unwrap_err()
                .to_string();
            assert!(error.contains("through symlink"), "{error}");
            assert!(!outside.join("css/site-editor.css").exists());
        }
    ...
    fn extracts_bundled_wordpress_without_overwriting_existing_files() {
        ...
        let archive = wordpress_archive(&repo_root, "6.3").unwrap();

        extract_wordpress_with_static_overlay_without_overwriting(&repo_root, &archive, &dir)
            .unwrap();

        assert!(contains_wordpress_files(&dir));
        assert!(dir.join("wp-load.php").is_file());
        assert!(dir.join("wp-admin/js/widgets.js").is_file());
        assert_eq!(
    ...
    fn validates_wordpress_zip_structure() {
        ...
        validate_wordpress_archive(&valid).unwrap();
        assert!(validate_wordpress_archive(&missing)
            .unwrap_err()
            .to_string()
            .contains("missing required entry"));
        assert!(validate_wordpress_archive(&corrupt)
            .unwrap_err()
            .to_string()
            .contains("Failed to read WordPress ZIP"));

        let destination = dir.join("zip-wordpress");
        extract_wordpress_archive_without_overwriting(&valid, &destination).unwrap();
        assert!(destination.join("wp-admin/install.php").is_file());

        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn validates_and_extracts_wordpress_tar_zst_structure() {
        let dir = temp_dir("validate-wp-tar-zst");
        let valid = dir.join("wp-6.9.tar.zst");
        write_tar_zst(
            &valid,
            &[
                ("wp-config-sample.php", b"<?php"),
                ...
                ("wp-includes/version.php", b"<?php"),
                ("wp-content/index.php", b"<?php"),
                ("wp-content/upgrade/", b""),
                ("index.php", b"archive"),
            ],
        );
        let missing = dir.join("wp-missing.tar.zst");
        write_tar_zst(&missing, &[("wp-load.php", b"<?php")]);
        let corrupt = dir.join("wp-corrupt.tar.zst");
        fs::write(&corrupt, b"not zstd").unwrap();
        ...
        assert!(validate_wordpress_archive(&missing)
            ...
            .to_string()
            .contains("missing required entry"));
        let corrupt_error = validate_wordpress_archive(&corrupt)
            .unwrap_err()
            .to_string();
        assert!(
            corrupt_error.contains("WordPress tar.zst")
                && (corrupt_error.contains("Failed to decompress")
                    || corrupt_error.contains("Failed to read")
                    || corrupt_error.contains("unreadable")),
            "{corrupt_error}"
        );

        let destination = dir.join("wordpress");
        fs::create_dir_all(&destination).unwrap();
        fs::write(destination.join("index.php"), b"existing").unwrap();
        extract_wordpress_archive_without_overwriting(&valid, &destination).unwrap();
        assert_eq!(
            fs::read(destination.join("index.php")).unwrap(),
            b"existing"
        ...
        assert!(destination.join("wp-admin/install.php").is_file());
        assert!(destination.join("wp-content/upgrade").is_dir());

        let wrong_types = dir.join("wp-wrong-types.tar.zst");
        write_tar_zst(
            &wrong_types,
            &[
                ("wp-config-sample.php", b"<?php"),
                ...
                ("wp-admin/install.php", b"<?php"),
                ("wp-includes/version.php", b"<?php"),
                ("wp-content", b"not a directory"),
            ],
        );
        let wrong_type_error = validate_wordpress_archive(&wrong_types)
            .unwrap_err()
            .to_string();
        assert!(wrong_type_error.contains("missing required entry wp-load.php"));

        for unsafe_path in ["../escape.php", "/absolute.php", "..\\escape.php"] {
            assert!(validate_tar_entry_path(PathBuf::from(unsafe_path).as_path(), &valid).is_err());
        }

        let symlink = dir.join("wp-symlink.tar.zst");
        write_tar_zst_symlink(&symlink);
        assert!(validate_wordpress_archive(&symlink)
            .unwrap_err()
            .to_string()
