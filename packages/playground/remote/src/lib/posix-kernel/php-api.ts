...
/**
 * Extra `-d` flags for every direct `php` spawn, mirroring the dinit
 * php-fpm command in `vfs-builder.ts` so both PHP paths behave identically:
 *
 *  - `extension=zip.so` / `curl.so`: load the libzip / libcurl side
 *    modules (kandelo #647 / #648) so blueprint steps' `unzipFile`
 *    (`ZipArchive`) and `curl_*` / `CURLFile` resolve.
 *  - `extension=phar.so`: load the Phar side module so the `wp-cli`
 *    blueprint step can run `wp-cli.phar` (needs the `Phar` class).
 *  - `curl.cainfo`: the WASM `curl.so` ships no CA bundle, so it needs an
 *    explicit one to verify HTTPS.
 *  - `zend.max_allowed_stack_size`: raise a graceful PHP fatal on deep
 *    recursion before V8's ~50-frame budget throws a RangeError. See
 *    `vfs-builder.ts` `buildServices()` for the budget math.
 */
...
export class KernelLimitedPHPApi {
	...
	async cp(fromPath: string, toPath: string): Promise<void> {
		await this.adapter.cp(this.toVfs(fromPath), this.toVfs(toPath));
	}
	...
	async run(request: PHPRunOptions): Promise<PHPResponse> {
		const response = await this.runCli(request);
	...
	/**
	 * Streamed sibling of {@link run}. The kernel spawn buffers the full
	 * output, so the streams deliver a single final chunk. Matching the
	 * `PHP.runStream` contract, a non-zero exit code is reported through
	 * `exitCode` instead of a thrown error — callers such as
	 * `runUnzipFileWithProgress` drain the streams before checking it.
	 */
	async runStream(request: PHPRunOptions): Promise<StreamedPHPResponse> {
		return StreamedPHPResponse.fromPHPResponse(await this.runCli(request));
	}
	...
	async request(request: PHPRequest): Promise<PHPResponse> {
		...
		if (bridgeResponse.status === 504) {
			logger.warn(
				`Request to ${unscoped} timed out at the gateway (HTTP 504): ` +
	...
	private async runCli(request: PHPRunOptions): Promise<PHPResponse> {
