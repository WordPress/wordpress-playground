...
export async function bootPlaygroundRemote() {
	...
	const playgroundApi = {
		...
		async captureInstalledDatabase() {
			return await kernelWorkerApi.captureInstalledDatabase();
		},
		// Test-only: sibling of `captureInstalledDatabase`. Rebuilds the full
		// VFS image with the just-captured install seeded in, so every other
		// test boots from the image and skips the WP-core/static extraction
		// entirely (not just the installer POST).
		async captureVfsImage() {
			return await kernelWorkerApi.captureVfsImage();
		},
		async boot(options: any) {
			// Read the e2e snapshot URL from the iframe URL (iframe scope has
			// `location`) so it stays out of the shared client boot options.
			// `getRemoteUrl` puts it there from a window global set by the
			// Playwright fixtures.
			const bootParams = new URLSearchParams(location.search);
			const preinstalledDb = bootParams.get('preinstalledDb');
			if (preinstalledDb) {
				options.preinstalledDatabaseUrl ??= preinstalledDb;
			}
			// Fast path: a full prebuilt VFS image lets the worker skip the
			// zip download + VFS build entirely (falls back to the DB seed
			// above if the image is missing/404). See `getRemoteUrl`.
			const prebuiltVfsImage = bootParams.get('prebuiltVfsImage');
			if (prebuiltVfsImage) {
				options.prebuiltVfsImageUrl ??= prebuiltVfsImage;
			}
