...
describe('PlaygroundWorkerEndpointBlueprintsV1', () => {
	...
	it('rejects legacy remote Blueprint v2 runner boot options', async () => {
		const bootWordPress = vi.fn();
		let endpoint:
			| {
					boot(options: Record<string, unknown>): Promise<void>;
	...
	it('passes caller-provided WordPress ZIP buffers to bootWordPress', async () => {
		const bootWordPress = vi.fn();
		const requestHandler = {
		...
		const zip = new TextEncoder().encode('zip').buffer;
		await endpoint.boot({
			scope: 'test',
			phpVersion: '8.3',
		...
		const passedZip = bootWordPress.mock.calls[0][1].wordPressZip;
		expect(passedZip).toBeInstanceOf(File);
		expect(await passedZip.text()).toBe('zip');
