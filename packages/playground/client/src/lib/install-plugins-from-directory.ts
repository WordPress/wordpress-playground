  ...
  await new Promise((finish) => {
    for (const preinstallPlugin of pluginsZipNames) {
      downloads.enqueue(async () => {
        let response = await fetch('/plugin-proxy?plugin=' + preinstallPlugin);
        if (progress) {
          response = cloneResponseMonitorProgress(
            response,
            progress.partialObserver(
              progressBudgetPerPlugin * 0.66,
              `Installing ${zipNameToHumanName(preinstallPlugin)} plugin...`
            )
          );
        }
  ...
  async #run() {
    ...
    try {
      ...
      while (this.#queue.length) {
        ...
        ++this.#_resolved;
        this.dispatchEvent(new CustomEvent('resolved', { detail: result }));
      }
    } finally {
      this.#running = false;
