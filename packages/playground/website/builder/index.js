...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const saveMethod = () => {
    const dataUri = `data:application/json;base64,${btoa(JSON.stringify(JSON.parse(editor.getValue()), null, 2))}`
    const link = document.createElement('a');
    link.setAttribute('href', dataUri);
    link.setAttribute('download', `blueprint-${(new Date).toISOString()}.json`);
    link.click();
  };
    ...
    input.addEventListener('change', event => {
      [...input.files].forEach(f => {
        ...
        reader.addEventListener('load', () => {
          editor.setValue(JSON.stringify(JSON.parse(reader.result), null, 2));
