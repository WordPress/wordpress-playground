...
describe('createRelayUrlRewriter — HTML', () => {
	...
	it('rewrites a full URL pointing at the original host', () => {
		...
		expect(out).toContain(
			`href="${PREFIX}/wp-admin/edit.php?post=1"`
		);
	});
	...
	it('does NOT touch URLs inside <script> bodies', () => {
		// Script content is data, not navigation. Rewriting a URL
		// inside a JS string literal would silently corrupt the script.
		const html =
			'<html><body><script>var u = "/wp-admin/admin-ajax.php"; fetch(u);</script></body></html>';
		const out = rw.rewriteHtml(html);
		expect(out).toContain('"/wp-admin/admin-ajax.php"');
		expect(out).not.toContain(`${PREFIX}/wp-admin/admin-ajax.php`);
	});
