...
export class PreviewService {
	...
	static installPreview(playground: any) {
		const preloader = this.preloader ? new Uint8Array(this.preloader) : 'Loading Resources...';
		const postloader = this.postloader ? new Uint8Array(this.postloader) : 'Activating Plugin...';
		...
		return playground
			...
			.then(() => playground.writeFile('/wordpress/collector-activate.html', postloader))
