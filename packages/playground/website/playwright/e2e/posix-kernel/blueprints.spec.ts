...
// posix-kernel mode ships a single PHP build (8.3) and loads `intl` as a
// dynamic side-module only when a blueprint opts in with
// `features: { intl: true }`. That opt-in makes the boot skip the shared
// (intl-off) snapshot and cold-build the image with `intl.so` staged (see
// `vfs-builder.ts` / `playground-worker-endpoint.ts` `withIntl`), so this
// test is slower than the snapshot-served ones but runs on the first try.
		...
		steps: [
			{
				...
				data: `<?php
					...
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, "https://httpbingo.org/post");
					...
					if ($error) {
						...
						if (isset($decoded['files']['file'])) {
							...
							$uploaded = $decoded['files']['file'];
							if (is_array($uploaded)) { $uploaded = implode('', $uploaded); }
							echo " CONTENT_MATCH:" . (strpos($uploaded, 'PLAYGROUND_TEST_CONTENT') !== false ? 'YES' : 'NO');
						} else {
