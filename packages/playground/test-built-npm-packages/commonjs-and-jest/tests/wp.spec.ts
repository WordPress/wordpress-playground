...
describe(`Wasmtime PHP ${WASMTIME_PHP_VERSION}`, () => {
	it('WordPress should load', async () => {
		const cli = await runCLI({
			command: 'server',
			php: WASMTIME_PHP_VERSION,
 ...
 *
 * This remains separate coverage for the legacy @php-wasm/node loader and is
 * intentionally not constrained to the Wasmtime CLI's PHP version.
 *
