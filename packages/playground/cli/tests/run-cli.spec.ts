...
describeUnix('native spawn', () => {
	async function bootCli(nativeSpawn = false) {
		return await runCLI({
			command: 'server',
			workers: 1,
			nativeSpawn,
	...
	test('uses the native host cwd when nativeSpawn is enabled', async () => {
		await using cliServer = await bootCli(true);
		await cliServer.playground.chdir('/wordpress');

		const result = await cliServer.playground.run({
			code: `<?php echo shell_exec('pwd');`,
		});

		expect(result.text.trim()).toBe(process.cwd());
	});

	test('keeps the VFS cwd for the default sandboxed spawn handler', async () => {
		await using cliServer = await bootCli();
		await cliServer.playground.chdir('/wordpress');
		...
		expect(result.text.trim()).toBe('/wordpress');
	});
});
