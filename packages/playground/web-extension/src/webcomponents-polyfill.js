	...
	(function () {
		...
		if (c || v) {
			a.a = function (a, b) {
				...
				this.R && this.R(c);
				b &&
					(m.call(c.content, f.call(a.content, !0)),
					lb(c.content, a.content));
				return c;
			};
			var lb = function (c, d) {
					if (
	...
	function ab(a, b) {
		...
		if (la(b))
			if (
				...
				for (a = 0; void 0 === this.o && a < b.length; a++)
					bb(this, b[a], a);
...
*/
	...
	ob &&
		...
		(window.ShadyDOM.force = ob));
	var pb = y.register || y.ce;
	pb &&
		window.customElements &&
		(window.customElements.forcePolyfill = pb); /*
...
*/
	...
	function Uc(a, b, c) {
		...
		if (!b.__noInsertionPoint) {
			...
			f &&
				...
				g.a.push.apply(
					g.a,
					[].concat(d instanceof Array ? d : ja(ia(d)))
				));
		}
	...
	function Yc(a) {
		...
		a &&
			'slot' === a.localName &&
			(b =
				(b = (b = A(a)) && b.L) && b.length ? b[0] : Yc(a.nextSibling));
		return b;
	}
	function Xc(a) {
			...
			void 0 === c &&
				(C(a)
					...
					: ((c = (c = a.parentNode) ? gd(c) : a),
					  D.contains.call(document.documentElement, a) &&
							(b.V = c)));
			return c;
		}
	}
	...
	function ld(a, b) {
		if (a.ownerDocument !== document)
			return D.importNode.call(document, a, b);
	...
	function zd(a, b, c) {
		if (b) {
			...
			k &&
				((e = wd(k, g, a, d, e, f)),
				-1 < e &&
					((h = k.splice(e, 1)[0].gb), k.length || (b[md] = void 0)));
	...
	function Fd(a, b, c, d) {
		...
		if (b == a.length && d == c.length) {
			h = a.length;
			for (var m = c.length, n = 0; n < k - g && Gd(a[--h], c[--m]); )
				n++;
			h = n;
		}
		e += g;
		...
		a.parent &&
			...
			a.atRule
				? 0 === c.indexOf('@media')
					...
					: c.match(rf) &&
					  ((a.type = sf),
					  (a.keyframesName = a.selector.split(kf).pop()))
	...
	function Of(a, b, c) {
		...
		Pf
			? a.compareDocumentPosition(Pf) ===
					Node.DOCUMENT_POSITION_PRECEDING && (Pf = a)
			: (Pf = a);
	}
	function Qf(a, b) {
		var c = a.indexOf('var(');
				...
				switch (k) {
					...
					case 'scheme':
						if (l && G.test(l)) p += l.toLowerCase();
							...
							void 0 != l &&
								h('Code point not allowed in scheme: ' + l);
							break a;
						} else {
							p = '';
							v = 0;
					...
					case 'no scheme':
						if (g && void 0 !== m[g.g]) {
							...
							h('Missing scheme.'), f.call(this), (this.h = !0);
						break;
