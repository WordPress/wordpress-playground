...
var require_requests_transport_fetch = __commonJS({
  "packages/wordpress-wasm/src/requests_transport_fetch.php"(exports, module2) {
    module2.exports = `<?php
/**
...
var import_php_wasm3 = require("php-wasm");
...
// packages/wordpress-wasm/src/worker-thread.js
(0, import_php_wasm_browser2.initializeWorkerThread)(startWordPress);
var DOCROOT = "/wordpress";
async function startWordPress({ absoluteUrl }) {
  const [phpLoaderModule, wpLoaderModule] = await Promise.all([
    import(`/php.js?${import_php_wasm2.phpJsHash}`),
    import(`/wp.js?${wpJsCacheBuster}`)
  ]);
  const php = await (0, import_php_wasm_browser2.loadPHPWithProgress)(phpLoaderModule, [wpLoaderModule]);
  patchWordPressFiles(php, absoluteUrl);
  const server = new import_php_wasm3.PHPServer(php, {
    documentRoot: DOCROOT,
    absoluteUrl,
    isStaticFilePath: isUploadedFilePath
  });
  return new import_php_wasm3.PHPBrowser(server);
}
