...
import { parseNumstat } from './parse-numstat.mjs';

// Run locally with:
//   node --test "packages/meta/src/pr-labels/**/*.test.mjs"

test('sums added + deleted into lines and keeps the path', () => {
	assert.deepEqual(parseNumstat('40\t2\tpackages/php-wasm/web/index.ts'), [
...
test('binary files ("-" counts) become 0 lines, not NaN', () => {
	...
	const [entry] = parseNumstat('-\t-\tpackages/php-wasm/web-builds/php.wasm');
...
test('ignores blank lines and a trailing newline', () => {
	assert.deepEqual(parseNumstat('3\t3\ta.ts\n'), [
		{ path: 'a.ts', lines: 6 },
	]);
});
