		...
		for (const configPlugin of documenterConfig.configFile.plugins || []) {
			try {
				...
				for (const featureName of configPlugin.enabledFeatureNames) {
					...
					if (
						featureDefinition.kind === 'MarkdownDocumenterFeature'
					) {
						if (this.markdownDocumenterFeature) {
							throw new Error(
								'A MarkdownDocumenterFeature is already loaded'
							);
						}
						...
						try {
							markdownDocumenterFeature =
								new featureDefinition.subclass(initialization);
						} catch (e) {
							throw new Error(
						...
						if (
							!(
								markdownDocumenterFeature instanceof
								MarkdownDocumenterFeature
							)
						) {
							throw new Error(
						...
						this.markdownDocumenterFeature =
							markdownDocumenterFeature;
					} else {
						throw new Error(
