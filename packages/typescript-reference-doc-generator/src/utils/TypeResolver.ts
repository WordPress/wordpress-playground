import { ApiItem, ApiModel, ExcerptToken, ExcerptTokenKind, IResolveDeclarationReferenceResult } from '@microsoft/api-extractor-model';
...
export default class TypeResolver {
	...
	public resolveTypeDocumentation(type: ExcerptToken | string): DocumentedType | null {
	...
	public resolveNativeToken(token: string): DocumentedType | null {
		...
		const toComparables = [(token: string) => token, (token: string) => token.toLowerCase()];
...
const NativeTypes: Record<string, string> = {
	...
	unknown: 'https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#any',
