// THE EXPANDING WAY OF DEBUGGING
// The next step here is to expand zend_fetch_dimension_address_read from Zend/zend_execute.c
// It's super inefficient so let's put it on hold for now
...
ZEND_API zend_result ZEND_FASTCALL zend_ast_evaluate2(zval *result, zend_ast *ast, zend_class_entry *scope)
{
	...
	switch (ast->kind) {
		...
		case ZEND_AST_DIM:
			...
			if (UNEXPECTED(zend_ast_evaluate2(&op1, ast->child[0], scope) != SUCCESS)) {
				ret = FAILURE;
			} else if (UNEXPECTED(zend_ast_evaluate2(&op2, ast->child[1], scope) != SUCCESS)) {
				zval_ptr_dtor_nogc2(&op1);
				ret = FAILURE;
			} else {
				zend_fetch_dimension_const(result, &op1, &op2, (ast->attr & ZEND_DIM_IS) ? BP_VAR_IS : BP_VAR_R);
//
//				zval_ptr_dtor_nogc2(&op1);
//				zval_ptr_dtor_nogc2(&op2);
...
ZEND_API zend_result zval_update_constant_ex2(zval *p, zend_class_entry *scope) /* {{{ */
{
	if (Z_TYPE_P(p) == IS_CONSTANT_AST) {
		...
		if (ast->kind == ZEND_AST_CONSTANT) {
			...
			if (UNEXPECTED(zend_ast_evaluate2(&tmp, ast, scope) != SUCCESS)) {
				return FAILURE;
			}
			zval_ptr_dtor_nogc2(p);
