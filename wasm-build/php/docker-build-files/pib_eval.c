...
ZEND_API const zend_internal_function zend_pass_function2 = {
...
ZEND_API void zend_vm_stack_init2(void)
{
...
ZEND_API void* zend_vm_stack_extend2(size_t size)
{
	zend_vm_stack stack;
	void *ptr;
...
static zend_never_inline ZEND_COLD void zend_throw_auto_init_in_prop_error2(zend_property_info *prop, const char *type) {
...
static zend_never_inline ZEND_COLD void zend_throw_auto_init_in_ref_error2(zend_property_info *prop, const char *type) {
	zend_string *type_str = zend_type_to_string(prop->type);
	zend_type_error(
...
static zend_never_inline ZEND_COLD void zend_throw_access_uninit_prop_by_ref_error2(
...
static ZEND_COLD void zend_verify_type_error_common2(
...
ZEND_API ZEND_COLD void zend_verify_arg_error2(
		const zend_function *zf, const zend_arg_info *arg_info, int arg_num, zval *value)
{
	...
	zend_verify_type_error_common2(
...
static zend_bool zend_verify_weak_scalar_type_hint2(uint32_t type_mask, zval *arg)
{
	zend_long lval;
	double dval;
	zend_string *str;
...
ZEND_API zend_bool zend_verify_scalar_type_hint2(uint32_t type_mask, zval *arg, zend_bool strict, zend_bool is_internal_arg)
{
...
#endif
	return zend_verify_weak_scalar_type_hint2(type_mask, arg);
}

ZEND_COLD zend_never_inline void zend_verify_property_type_error2(zend_property_info *info, zval *property)
{
...
static zend_always_inline zend_bool i_zend_check_property_type(zend_property_info *info, zval *property, zend_bool strict)
{
	...
	return zend_verify_scalar_type_hint2(type_mask, property, strict, 0);
}
...
static zend_always_inline zend_bool i_zend_verify_property_type(zend_property_info *info, zval *property, zend_bool strict)
{
	...
	zend_verify_property_type_error2(info, property);
	return 0;
}

ZEND_API zend_bool zend_never_inline zend_verify_property_type2(zend_property_info *info, zval *property, zend_bool strict) {
...
ZEND_API zend_bool zend_value_instanceof_static2(zval *zv) {
	if (Z_TYPE_P(zv) != IS_OBJECT) {
		return 0;
	}
...
builtin_types:
	...
	if ((type_mask & MAY_BE_STATIC) && zend_value_instanceof_static2(arg)) {
		return 1;
	}
	...
	return zend_verify_scalar_type_hint2(type_mask, arg,
...
static zend_always_inline bool zend_verify_recv_arg_type(zend_function *zf, uint32_t arg_num, zval *arg, void **cache_slot)
{
	...
	if (ZEND_TYPE_IS_SET(cur_arg_info->type)
		...
		zend_verify_arg_error2(zf, cur_arg_info, arg_num, arg);
		return 0;
	}
		...
		zend_function *zf, zend_arg_info *arg_info, uint32_t arg_num, zval *arg, void **cache_slot)
{
	...
	if (UNEXPECTED(!zend_check_type(arg_info->type, arg, cache_slot, zf->common.scope, 0, 0))) {
		zend_verify_arg_error2(zf, arg_info, arg_num, arg);
		return 0;
	}
...
ZEND_API ZEND_COLD void zend_verify_return_error2(const zend_function *zf, zval *value)
{
...
static ZEND_COLD void zend_verify_missing_return_type(const zend_function *zf)
{
	...
	zend_verify_return_error2(zf, NULL);
}
...
static zend_never_inline void zend_binary_assign_op_typed_prop(zend_property_info *prop_info, zval *zptr, zval *value OPLINE_DC EXECUTE_DATA_DC)
{
	...
	if (EXPECTED(zend_verify_property_type2(prop_info, &z_copy, EX_USES_STRICT_TYPES()))) {
		zval_ptr_dtor(zptr);
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_deprecated_function2(const zend_function *fbc)
{
	if (fbc->common.scope) {
...
static void zend_incdec_typed_prop(zend_property_info *prop_info, zval *var_ptr, zval *copy OPLINE_DC EXECUTE_DATA_DC)
{
	...
	} else if (UNEXPECTED(!zend_verify_property_type2(prop_info, var_ptr, EX_USES_STRICT_TYPES()))) {
		zval_ptr_dtor(var_ptr);
		ZVAL_COPY_VALUE(var_ptr, copy);
		ZVAL_UNDEF(copy);
	} else if (copy == &tmp) {
...
ZEND_API ZEND_COLD zend_result ZEND_FASTCALL zend_undefined_offset_write2(HashTable *ht, zend_long lval)
{
...
ZEND_API ZEND_COLD zend_result ZEND_FASTCALL zend_undefined_index_write2(HashTable *ht, zend_string *offset)
{
	/* The array may be destroyed while throwing the notice.
	 * Temporarily increase the refcount to detect this situation. */
	if (!(GC_FLAGS(ht) & IS_ARRAY_IMMUTABLE)) {
...
ZEND_COLD void ZEND_FASTCALL zend_param_must_be_ref2(const zend_function *func, uint32_t arg_num)
{
...
num_undef:
		switch (type) {
			...
			case BP_VAR_IS:
				...
				break;
			case BP_VAR_RW:
				if (UNEXPECTED(zend_undefined_offset_write2(ht, hval) == FAILURE)) {
					return NULL;
				}
				/* break missing intentionally */
			case BP_VAR_W:
		...
		if (retval) {
				...
				if (UNEXPECTED(Z_TYPE_P(retval) == IS_UNDEF)) {
					switch (type) {
						...
						case BP_VAR_IS:
							...
							break;
						case BP_VAR_RW:
							if (UNEXPECTED(zend_undefined_index_write2(ht, offset_key))) {
								return NULL;
							}
							/* break missing intentionally */
						case BP_VAR_W:
			...
			switch (type) {
				...
				case BP_VAR_RW:
					...
					if (UNEXPECTED(zend_undefined_index_write2(ht, offset_key) == FAILURE)) {
...
ZEND_API void zend_fetch_dimension_const2(zval *result, zval *container, zval *dim, int type)
{
...
ZEND_API zend_bool zend_verify_ref_array_assignable2(zend_reference *ref) {
	zend_property_info *prop;
	ZEND_ASSERT(ZEND_REF_HAS_TYPE_SOURCES(ref));
	ZEND_REF_FOREACH_TYPE_SOURCES(ref, prop) {
		if (!check_type_array_assignable(prop->type)) {
			zend_throw_auto_init_in_ref_error2(prop, "array");
			return 0;
		}
	} ZEND_REF_FOREACH_TYPE_SOURCES_END();
	return 1;
		...
		case ZEND_FETCH_DIM_WRITE:
			if (promotes_to_array(ptr)) {
				...
				if (!check_type_array_assignable(prop_info->type)) {
					zend_throw_auto_init_in_prop_error2(prop_info, "array");
			...
			if (Z_TYPE_P(ptr) != IS_REFERENCE) {
				...
				if (Z_TYPE_P(ptr) == IS_UNDEF) {
					if (!ZEND_TYPE_ALLOW_NULL(prop_info->type)) {
						zend_throw_access_uninit_prop_by_ref_error2(prop_info);
						if (result) ZVAL_ERROR(result);
						return 0;
					}
					ZVAL_NULL(ptr);
...
ZEND_API ZEND_COLD void zend_throw_ref_type_error_type2(zend_property_info *prop1, zend_property_info *prop2, zval *zv) {
...
ZEND_API ZEND_COLD void zend_throw_ref_type_error_zval2(zend_property_info *prop, zval *zv) {
	zend_string *type_str = zend_type_to_string(prop->type);
...
ZEND_API ZEND_COLD void zend_throw_conflicting_coercion_error2(zend_property_info *prop1, zend_property_info *prop2, zval *zv) {
	zend_string *type1_str = zend_type_to_string(prop1->type);
	zend_string *type2_str = zend_type_to_string(prop2->type);
...
ZEND_API zend_bool ZEND_FASTCALL zend_verify_ref_assignable_zval2(zend_reference *ref, zval *zv, zend_bool strict)
{
	zend_property_info *prop;
...
type_error:
		...
		if (result < 0) {
			if (!first_prop) {
				...
				if (!zend_verify_weak_scalar_type_hint2(
				...
				zval tmp;
				ZVAL_COPY(&tmp, zv);
				if (!zend_verify_weak_scalar_type_hint2(ZEND_TYPE_FULL_MASK(prop->type), &tmp)) {
					zval_ptr_dtor(&tmp);
					goto type_error;
				}
...
conflicting_coercion_error:
				zend_throw_conflicting_coercion_error2(first_prop, prop, zv);
				zval_ptr_dtor(&coerced_value);
				return 0;
			}
		}
...
ZEND_API zval* zend_assign_to_typed_ref2(zval *variable_ptr, zval *orig_value, zend_uchar value_type, zend_bool strict)
{
	zend_bool ret;
	zval value;
...
ZEND_API zend_bool ZEND_FASTCALL zend_verify_prop_assignable_by_ref2(zend_property_info *prop_info, zval *orig_val, zend_bool strict) {
	zval *val = orig_val;
	if (Z_ISREF_P(val) && ZEND_REF_HAS_TYPE_SOURCES(Z_REF_P(val))) {
		...
		if (result < 0) {
			...
			zval tmp;
			ZVAL_COPY(&tmp, val);
			if (zend_verify_weak_scalar_type_hint2(ZEND_TYPE_FULL_MASK(prop_info->type), &tmp)) {
	...
	zend_verify_property_type_error2(prop_info, val);
	return 0;
}

ZEND_API void ZEND_FASTCALL zend_ref_add_type_source2(zend_property_info_source_list *source_list, zend_property_info *prop)
{
...
ZEND_API void ZEND_FASTCALL zend_ref_del_type_source2(zend_property_info_source_list *source_list, zend_property_info *prop)
{
...
ZEND_API void execute_internal2(zend_execute_data *execute_data, zval *return_value)
{
	execute_data->func->internal_function.handler(execute_data, return_value);
}

ZEND_API void zend_clean_and_cache_symbol_table2(zend_array *symbol_table) /* {{{ */
{
...
ZEND_API void zend_free_compiled_variables2(zend_execute_data *execute_data) /* {{{ */
{
...
ZEND_API zend_function * ZEND_FASTCALL zend_fetch_function2(zend_string *name) /* {{{ */
{
...
ZEND_API zend_function * ZEND_FASTCALL zend_fetch_function_str2(const char *name, size_t len) /* {{{ */
{
...
ZEND_API void ZEND_FASTCALL zend_init_func_run_time_cache2(zend_op_array *op_array) /* {{{ */
{
...
ZEND_API void zend_init_func_execute_data2(zend_execute_data *ex, zend_op_array *op_array, zval *return_value) /* {{{ */
{
#if defined(ZEND_VM_FP_GLOBAL_REG) && ((ZEND_VM_KIND == ZEND_VM_KIND_CALL) || (ZEND_VM_KIND == ZEND_VM_KIND_HYBRID))
...
ZEND_API void zend_init_code_execute_data2(zend_execute_data *execute_data, zend_op_array *op_array, zval *return_value) /* {{{ */
{
	EX(prev_execute_data) = EG(current_execute_data);
...
ZEND_API void zend_init_execute_data2(zend_execute_data *execute_data, zend_op_array *op_array, zval *return_value) /* {{{ */
{
	if (EX_CALL_INFO() & ZEND_CALL_HAS_SYMBOL_TABLE) {
...
zend_execute_data *zend_vm_stack_copy_call_frame2(zend_execute_data *call, uint32_t passed_args, uint32_t additional_args) /* {{{ */
{
	zend_execute_data *new_call;
	int used_stack = (EG(vm_stack_top) - (zval*)call) + additional_args;

	/* copy call frame into new stack segment */
	new_call = zend_vm_stack_extend2(used_stack * sizeof(zval));
	*new_call = *call;
...
ZEND_API void zend_cleanup_unfinished_execution2(zend_execute_data *execute_data, uint32_t op_num, uint32_t catch_op_num) {
...
zval * ZEND_FASTCALL zend_handle_named_arg2(
...
ZEND_API void ZEND_FASTCALL zend_free_extra_named_params2(zend_array *extra_named_params)
{
...
#if defined(ZEND_VM_IP_GLOBAL_REG) && ((ZEND_VM_KIND == ZEND_VM_KIND_CALL) || (ZEND_VM_KIND == ZEND_VM_KIND_HYBRID))
/* Special versions of functions that sets EX(opline) before calling zend_vm_stack_extend2() */
static zend_always_inline zend_execute_data *_zend_vm_stack_push_call_frame_ex(uint32_t used_stack, uint32_t call_info, zend_function *func, uint32_t num_args, void *object_or_called_scope) /* {{{ */
{
	...
	if (UNEXPECTED(used_stack > (size_t)(((char*)EG(vm_stack_end)) - (char*)call))) {
		...
		call = (zend_execute_data*)zend_vm_stack_extend2(used_stack);
		ZEND_ASSERT_VM_STACK_GLOBAL;
