...
When this skill starts, run these commands and use their output as context:

- `git branch --show-current` for the current branch
- `git rev-parse --short HEAD` for the current commit
- `git status -sb` for remote status

## User Instructions

Treat the user's prompt as the instruction source for this run.
...
Parse any overrides from the user's prompt and merge with defaults.
...
2. **ci-monitor-subagent**: calls one MCP tool (ci_information or update_self_healing_fix), returns structured result, exits
