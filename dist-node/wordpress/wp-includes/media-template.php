<?php
 function wp_underscore_audio_template() { $audio_types = wp_get_audio_extensions(); ?>
...
>
 ...
 endforeach; ?>
	...
	<?php
} function wp_underscore_video_template() { $video_types = wp_get_video_extensions(); ?>
<#  var w_rule = '', classes = [],
...
<video controls
	...
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
 $props = array( 'poster' => '', 'preload' => 'metadata', ); foreach ( $props as $key => $value ) : if ( empty( $value ) ) { ?>
		<#
...
>
 ...
 foreach ( $video_types as $type ) : ?>
	...
	<?php
} function wp_print_media_templates() { $class = 'media-modal wp-core-ui'; $alt_text_description = sprintf( __( '<a href="%1$s" %2$s>Learn how to describe the purpose of the image%3$s</a>. Leave empty if the image is purely decorative.' ), esc_url( 'https://www.w3.org/WAI/tutorials/images/decision-tree' ), 'target="_blank" rel="noopener"', sprintf( '<span class="screen-reader-text"> %s</span>', __( '(opens in a new tab)' ) ) ); ?>
 ...
 printf( __( '%1$s by %2$s pixels' ), '{{ data.width }}', '{{ data.height }}' ); ?>
			...
			<div class="settings">
				...
				<# if ( 'audio' === data.type ) { #>
				<?php
 foreach ( array( 'artist' => __( 'Artist' ), 'album' => __( 'Album' ), ) as $key => $label ) : ?>
		...
		<# if ( 'undefined' !== typeof data.sizes ) { #>
			<span class="setting">
				...
				<select id="attachment-display-settings-size" class="size" name="size"
					...
					<# } #>>
					<?php
 $sizes = apply_filters( 'image_size_names_choose', array( 'thumbnail' => __( 'Thumbnail' ), 'medium' => __( 'Medium' ), 'large' => __( 'Large' ), 'full' => __( 'Full Size' ), ) ); foreach ( $sizes as $value => $name ) : ?>
						<#
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<span class="setting size">
			...
			<select id="gallery-settings-size" class="size" name="size"
				...
				>
				<?php
 $size_names = apply_filters( 'image_size_names_choose', array( 'thumbnail' => __( 'Thumbnail' ), 'medium' => __( 'Medium' ), 'large' => __( 'Large' ), 'full' => __( 'Full Size' ), ) ); foreach ( $size_names as $size => $label ) : ?>
