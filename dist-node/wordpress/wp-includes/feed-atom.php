<?php
 header( 'Content-Type: ' . feed_content_type( 'atom' ) . '; charset=' . get_option( 'blog_charset' ), true ); $more = 1; echo '<?xml version="1.0" encoding="' . get_option( 'blog_charset' ) . '"?' . '>'; do_action( 'rss_tag_pre', 'atom' ); ?>
<feed
...
>
	...
	<?php
 do_action( 'atom_head' ); while ( have_posts() ) : the_post(); ?>
	<entry>
		<author>
 ...
 $author_url = get_the_author_meta( 'url' ); if ( ! empty( $author_url ) ) : ?>
		...
		<?php
 atom_enclosure(); do_action( 'atom_entry' ); if ( get_comments_number() || comments_open() ) : ?>
