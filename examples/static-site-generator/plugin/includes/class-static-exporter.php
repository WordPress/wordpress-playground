...
final class SSGWP_Static_Exporter {
	...
	public function export_to_directory( $output_dir, array $args = array() ) {
		...
		while ( ! empty( $queue ) && count( $exported ) < $max_pages ) {
			...
			$html        = $this->rewrite_document_urls( $response, $target_path, $args['url_mode'] );

			$this->write_file( trailingslashit( $output_dir ) . $target_path, $html );
			$exported[] = $url;

			if ( ! empty( $args['crawl_links'] ) ) {
				foreach ( $this->extract_links( $response, $url, $collector ) as $linked_url ) {
		...
		$this->rewrite_copied_text_assets( $output_dir, $args['url_mode'] );
	...
	/**
	 * Rewrite same-site absolute URLs in a document.
	 *
	 * @param string $document     Document contents.
	 * @param string $target_path  Relative target file path.
	 * @param string $url_mode     URL mode.
	 * @return string Rewritten document.
	 */
	private function rewrite_document_urls( $document, $target_path, $url_mode ) {
		$replacement_base = $this->replacement_base_for_path( dirname( $target_path ), $url_mode );

		return $this->replace_same_site_urls( $document, $replacement_base );
	}
	 ...
	 * @return string
	 */
	private function replacement_base_for_path( $relative_dir, $url_mode ) {
		if ( 'absolute' === $url_mode ) {
		...
		$depth = count( array_filter( explode( '/', $relative_dir ) ) );

		return str_repeat( '../', $depth );
	}

	/**
	 * Replace site, home, content, and includes URLs with a static base.
	 *
	 * @param string $content          File content.
	 * @param string $replacement_base Replacement base.
	 * @return string
	 */
	private function replace_same_site_urls( $content, $replacement_base ) {
		$bases = array_unique(
			array_filter(
				array(
					home_url( '/' ),
		...
		foreach ( $bases as $base ) {
			...
			$base_path  = isset( $parsed['path'] ) ? ltrim( $parsed['path'], '/' ) : '';
			$target     = $replacement_base . $base_path;
			$target     = preg_replace( '#/+#', '/', $target );
			$target     = str_replace( ':/', '://', $target );
			$escaped_in = str_replace( '/', '\\/', $base );
			$escaped_to = str_replace( '/', '\\/', $target );

			$content = str_replace( $base, $target, $content );
			...
			if ( $base_without_slash !== $base ) {
				$exact_target     = $target;
				$escaped_exact_in = str_replace( '/', '\\/', $base_without_slash );
				$escaped_exact_to = str_replace( '/', '\\/', $exact_target );
				$pattern          = '#' . preg_quote( $base_without_slash, '#' ) . '(?=([\'"\s<)]|$))#';
				$escaped_pattern  = '#' . preg_quote( $escaped_exact_in, '#' ) . '(?=([\'"\s<)]|$))#';

				$content = preg_replace( $pattern, $exact_target, $content );
				$content = preg_replace( $escaped_pattern, $escaped_exact_to, $content );
			}
		}
	 ...
	 * @return string[]
	 */
	private function extract_links( $html, $base_url, SSGWP_URL_Collector $collector ) {
		$links = array();
		...
		if ( class_exists( 'WP_HTML_Tag_Processor' ) ) {
			...
			while ( $processor->next_tag( 'A' ) ) {
				...
				$url = $collector->resolve_relative_url( $href, $base_url );
				$url = $collector->normalize_url( $url );
	 ...
	 * @param string $url_mode   URL mode.
	 */
	private function rewrite_copied_text_assets( $output_dir, $url_mode ) {
		$iterator = new RecursiveIteratorIterator(
		...
		foreach ( $iterator as $file ) {
			...
			$content  = $this->rewrite_document_urls( $content, $relative, $url_mode );
