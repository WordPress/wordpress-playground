...
function getStrictRequiresFilter({ strictRequires }) {
	switch (strictRequires) {
		...
		default:
			if (
				typeof strictRequires === 'string' ||
				Array.isArray(strictRequires)
			) {
				return {
...
async function getStaticRequireProxy(id, requireReturnsDefault, loadModule) {
	...
	if (!commonjsMeta) {
		...
		return `export { __moduleExports as default } from ${JSON.stringify(
			id
		)};`;
...
function getEsImportProxy(id, defaultIsModuleExports) {
	...
	let code =
		...
		`import { __require as ${requireModule} } from ${JSON.stringify(
			id
		)};\n` +
	...
	return {
		...
		async resolveId(importee, importer, resolveOptions) {
			...
			const currentlyResolvingForParent =
				currentlyResolving.get(importer);
			if (
	...
	return {
		...
		async shouldTransformCachedModule({
			...
			if (parentRequires) {
				...
				if (
					getTypeForFullyAnalyzedModule(parentId) !==
					parentMeta.isCommonJS
				) {
					return true;
				}
				for (const {
			...
			return (
				await Promise.all(
					Object.keys(resolvedSources)
						...
						.filter(
							({ id, external }) =>
								!(external || parentRequireSet.has(id))
						)
						.map(async (resolved) => {
		...
		resolveRequireSourcesAndUpdateMeta:
			...
			async (parentId, isParentCommonJS, parentMeta, sources) => {
				...
				fullyAnalyzedModules[parentId] = true;
				return requireTargets.map(
					({ id: dependencyId, allowProxy }, index) => {
		...
		isCurrentlyResolving(source, parentId) {
			const currentlyResolvingForParent =
				currentlyResolving.get(parentId);
			return (
				currentlyResolvingForParent &&
				currentlyResolvingForParent.has(source)
			);
		},
	};
}
...
function getExports(
	...
	if (!isRestorableCompiledEsm || defaultIsModuleExports === true) {
		...
		exports.push(
			`${deconflictedDefaultExportName || exportsName} as default`
		);
	} else {
		exportDeclarations.push(
...
function processRequireExpressions(
	...
	for (const { source, id: resolvedId, isCommonJS } of requireTargets) {
		...
		if (needsImport) {
			if (isCommonJS === IS_WRAPPED_COMMONJS) {
				imports.push(
					`import { __require as ${name} } from ${JSON.stringify(
						resolvedId
					)};`
				);
			} else {
				imports.push(
					`import ${
						usesRequired ? `${name} from ` : ''
					}${JSON.stringify(resolvedId)};`
				);
			}
		}
	}
			...
			switch (node.type) {
				case 'AssignmentExpression':
					if (node.left.type === 'MemberExpression') {
						...
						const exportsPatternMatch = exportsPattern.exec(
							flattened.keypath
						);
						if (
							!exportsPatternMatch ||
						...
						if (flattened.keypath === 'module.exports') {
							...
							if (defaultIsModuleExports === false) {
								...
								if (node.right.type === 'ObjectExpression') {
									...
									isRequireExpression(node.right, scope)
								) {
									shouldWrap = true;
									reexports = true;
								}
							...
							const exportsAssignments =
								exportsAssignmentsByName.get(exportName) || {
									nodes: [],
									scopes: new Set(),
								};
				...
				case 'CallExpression': {
					...
					if (!ignoreRequire(requireStringArg)) {
						...
						if (
							...
							for (const name of extractAssignedNames(
								parent.id
							)) {
				...
				case 'Identifier': {
					const { name } = node;
					if (!isReference(node, parent) || scope.contains(name))
						return;
					switch (name) {
						case 'require':
							uses.require = true;
...
function commonjs(options = {}) {
	...
	const isPossibleCjsId = (id) => {
		...
		return (
			extName === '.cjs' || (extensions.includes(extName) && filter(id))
		);
	};
	...
	function transformAndCheckExports(code, id) {
		...
		if (
			...
			console.log(
				...
				isEsModule,
				'&& ',
				!options.transformMixedEsModules,
				')'
			);
		...
		const needsRequireWrapper = !isEsModule && strictRequiresFilter(id);
	...
	return {
		...
		buildStart({ plugins }) {
			...
			const nodeResolve = plugins.find(
				({ name }) => name === 'node-resolve'
			);
			if (nodeResolve) {
				validateVersion(
					nodeResolve.version,
		...
		shouldTransformCachedModule(...args) {
			return requireResolver.shouldTransformCachedModule.call(
				this,
				...args
			);
		},
		...
		transform(code, id) {
			...
			console.log(id, 'transform commonjs');
			console.log(
				'calling',
				transformAndCheckExports.call(this, code, id).then((x) => {
					console.log({ yaay: x });
				})
			);
			try {
