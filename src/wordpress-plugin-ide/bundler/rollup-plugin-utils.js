...
function getMatcherPattern(id, resolutionBase) {
	return (
		getMatcherGlob(id, resolutionBase)
			.replace(/\*\*\//, '.*/')
			.replace(/(^|[^\*\.])\*/, '$1[^/]*?') + '$'
	);
}
...
function getMatcherGlob(id, resolutionBase) {
	...
	return pathJoin(basePath, normalizePath(id));
}

export function dataToEsm(data, options = {}) {
	const t = options.compact
		? ''
		: 'indent' in options
		? options.indent
		: '\t';
...
export function extractAssignedNames(param) {
	const names = [];
...
export function attachScopes(ast, propertyName = 'scope') {
	let scope = new Scope();
	walk(ast, {
		enter(n, parent) {
			const node = n;
