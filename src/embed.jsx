...
function App() {
	...
	const initialCode = useMemo(
		() => (
			b64_to_utf8( window.location.hash.substring( 1 ) ) ||
	...
	const handleRun = useCallback( () => {
		...
		async function doWork() {
			try {
				...
				const result = await runner( transpiledCode );
				console.log( result );
				setOutput( result );
			} catch ( e ) {
				console.error( e );
	...
	return (
		<>
			<Editor
				...
				initialLanguage="javascript"
				onRun={ handleRun }
				className={ `${ editorProps.className ? editorProps.className : '' } border-solid border border-indigo-600 flex-grow` }
				{ ...editorProps }
			/>
			...
			<div className="min-h-20 h-20">
				...
				{ ! isRunning && withInspector && output !== INITIAL_OUTPUT && (
