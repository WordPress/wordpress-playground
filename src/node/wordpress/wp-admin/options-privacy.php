...
<div class="privacy-settings-body hide-if-no-js">
 ...
 if ( $privacy_policy_page_exists ) { $edit_href = add_query_arg( array( 'post' => $privacy_policy_page_id, 'action' => 'edit', ), admin_url( 'post.php' ) ); $view_href = get_permalink( $privacy_policy_page_id ); ?>
				<strong>
				<?php
 if ( 'publish' === get_post_status( $privacy_policy_page_id ) ) { printf( __( '<a href="%1$s">Edit</a> or <a href="%2$s">view</a> your Privacy Policy page content.' ), esc_url( $edit_href ), esc_url( $view_href ) ); } else { printf( __( '<a href="%1$s">Edit</a> or <a href="%2$s">preview</a> your Privacy Policy page content.' ), esc_url( $edit_href ), esc_url( $view_href ) ); } ?>
				</strong>
			<?php
 } printf( __( 'Need help putting together your new Privacy Policy page? <a href="%1$s" %2$s>Check out our Privacy Policy guide%3$s</a> for recommendations on what content to include, along with policies suggested by your plugins and theme.' ), esc_url( admin_url( 'options-privacy.php?tab=policyguide' ) ), '', '' ); ?>
	</p>
	<hr>
	<?php
	...
	<table class="form-table tools-privacy-policy-page" role="presentation">
		<tr>
			<th scope="row">
				...
				<?php
 if ( $has_pages ) { _e( 'Create a new Privacy Policy Page' ); } else { _e( 'There are no pages.' ); } ?>
				</label>
			</th>
			<td>
				<form class="wp-create-privacy-page" method="post" action="">
					...
					<?php
 wp_nonce_field( 'create-privacy-page' ); submit_button( __( 'Create' ), 'secondary', 'submit', false, array( 'id' => 'create-page' ) ); ?>
				</form>
			</td>
		</tr>
		<?php if ( $has_pages ) : ?>
		<tr>
			<th scope="row">
				<label for="page_for_privacy_policy">
					<?php
 if ( $privacy_policy_page_exists ) { _e( 'Change your Privacy Policy page' ); } else { _e( 'Select a Privacy Policy page' ); } ?>
				</label>
			</th>
			<td>
				<form method="post" action="">
					...
					<?php
 wp_dropdown_pages( array( 'name' => 'page_for_privacy_policy', 'show_option_none' => __( '&mdash; Select &mdash;' ), 'option_none_value' => '0', 'selected' => $privacy_policy_page_id, 'post_status' => array( 'draft', 'publish' ), ) ); wp_nonce_field( 'set-privacy-page' ); submit_button( __( 'Use This Page' ), 'primary', 'submit', false, array( 'id' => 'set-page' ) ); ?>
				</form>
			</td>
		</tr>
		<?php endif; ?>
