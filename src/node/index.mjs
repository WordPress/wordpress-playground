...
export async function startWordPress( options ) {
	options = {
		...
		siteUrl: null,
		client: {},
		base64DbPath: path.join( __dirname, '/base64-encoded-database' ),
		...options,
	};

	if ( options.base64DbPath ) {
		initDatabaseFromBase64File( options.base64DbPath );
	}

	const wp = await createWordPressClient( options.client );

	if ( options.siteUrl ) {
		await wp.init( options.siteUrl );
	}
	...
	if ( options.install ) {
		...
		await install(
			browser,
			options.siteUrl,
			typeof options.install === 'boolean' ? {} : options.install,
		);
	}

	if ( options.initialUrl ) {
		await browser.request( { path: options.initialUrl } );
	}
