...
- Add a Playground CLI export smoke test and keep static exports from rewriting
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Include Playground CLI spawn errors and signals in static export smoke
  failure diagnostics.
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
...
- Require matching schemes when classifying static export URLs as same-site, so
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Show an interactive static export progress bar with the current action,
  percent complete, a completed-action log, and reload recovery in the
  Playground admin screen while the ZIP download is being prepared.
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
...
- Preserve same-origin `preconnect` and `dns-prefetch` resource hints in static
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Preserve static export resource hints in the regex HTML rewrite fallback
  used when WordPress' HTML API is unavailable.
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
...
- Treat semantic static export link relations such as `rel="author"` as
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Treat additional document-style static export link relations such as
  `rel="about"`, `rel="copyright"`, `rel="glossary"`, and `rel="payment"` as
...
- Treat additional schema.org identity metadata URLs such as `contributor` and
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Treat schema.org relationship metadata such as `about`, `mainEntity`,
  `mentions`, and `subjectOf` as crawlable static export page references.
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
...
- Copy same-site asset dependencies discovered while rewriting copied CSS
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Copy sibling file dependencies discovered while rewriting copied text assets,
  such as same-directory manifest icons.
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
...
- Copy WebVTT caption files discovered while rewriting linked copied text
  ...
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
- Copy source map files discovered from linked copied CSS and JavaScript
  assets while still excluding source maps from bulk asset directory copies.
  ([#3619](https://github.com/WordPress/wordpress-playground/pull/3619))
