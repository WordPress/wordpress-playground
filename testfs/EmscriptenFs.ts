...
import {
	...
	IndexFileInode,
	SyncIndexFS,
} from '@zenfs/core/backends/Index.js';
...
import { ApiError, Cred, SyncStoreFS } from '@zenfs/core';
...
function convertError(e: EmscriptenError, path: string = ''): ApiError {
	...
	return new ApiError(
		errno,
		ErrorStrings[errno],
		paths.length > 0 ? '/' + paths.join('/') : path
	);
}
 ...
 */
export class EmscriptenFileSystem extends SyncStoreFS {
	...
	public renameSync(oldPath: string, newPath: string, cred: Cred): void {
		try {
			...
			if (e.errno === ErrorCode.ENOENT) {
				throw convertError(
					e,
					this.existsSync(oldPath, cred) ? newPath : oldPath
				);
			} else {
	...
	public readFileSync(
		...
		try {
			const data: Uint8Array = this._FS.readFile(p, {
